...
namespace Server.TCP
{
    ...
    public sealed class SocketListener
    {
        ...
        private bool _SendAsync(SocketAsyncEventArgs writeEventArgs, out bool exception)
        {
            ...
            catch (Exception) //此处有可能是对象非法等异常, 例如Socket对象已经无效
            {
                LogManager.WriteLog(LogTypes.Error, string.Format("在SocketListener::_SendAsync 中发生了异常错误"));
                exception = true;
        ...
        private void ProcessAccept(SocketAsyncEventArgs e)
        {
            ...
            try
            {
                LogManager.WriteLog(LogTypes.Info, string.Format("新远程连接: {0}, 当前总共: {1}", s.RemoteEndPoint, numConnectedSockets));
            }
            catch (Exception)
            {
            }
