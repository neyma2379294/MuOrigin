...
namespace GameServer.Server.CmdProcesser
{
    ...
    public class SaleCmdsProcessor : ICmdProcessor
    {
        ...
        private bool MarketBuyGoods(GameClient client, string[] fields)
        {
            ...
            {
                if ((clientMoneyType == 1 && saleGoodsItem.SalingGoodsData.SaleMoney1 != clientMoneyValue) ||
                    (clientMoneyType == 2 && saleGoodsItem.SalingGoodsData.SaleYuanBao != clientMoneyValue))
                {
                    SaleGoodsManager.AddSaleGoodsItem(saleGoodsItem);
                ...
                {
                    if (!GameManager.ClientMgr.SubUserMoney(tcpMgr.MySocketListener, tcpClientPool, pool, client, saleGoodsItem.SalingGoodsData.SaleYuanBao,"新交易市场购买",false,false))
                    {
                        //返回管理
                        SaleGoodsManager.AddSaleGoodsItem(saleGoodsItem);
            ...
            else //离线购买
            {
                ...
                else //离线 摆摊逻辑
                {
                    ...
                    if ((clientMoneyType == 1 && liXianSaleGoodsItem.SalingGoodsData.SaleMoney1 != clientMoneyValue) ||
                        (clientMoneyType == 2 && liXianSaleGoodsItem.SalingGoodsData.SaleYuanBao != clientMoneyValue))
                    {
                        //返回管理
                        LiXianBaiTanManager.AddLiXianSaleGoodsItem(liXianSaleGoodsItem);
                    ...
                    {
                        if (!GameManager.ClientMgr.SubUserMoney(tcpMgr.MySocketListener, tcpClientPool, pool, client, liXianSaleGoodsItem.SalingGoodsData.SaleYuanBao, "新交易市场购买", false,false))
                        {
                            //返回管理
                            LiXianBaiTanManager.AddLiXianSaleGoodsItem(liXianSaleGoodsItem);
                        ...
                        {
                            //添加用户点卷
                            GameManager.ClientMgr.AddOfflineUserMoney(tcpMgr.MySocketListener, tcpClientPool, pool, liXianSaleGoodsItem.RoleID, CalcRealMoneyAfterTax(liXianSaleGoodsItem.SalingGoodsData.SaleYuanBao, MoneyTypes.YuanBao, out tax), "新交易市场出售(离线)", liXianSaleGoodsItem.ZoneID, liXianSaleGoodsItem.UserID);
                        }
                    }
