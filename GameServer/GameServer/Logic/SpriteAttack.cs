...
namespace GameServer.Logic
{
    ...
    class SpriteAttack
    {
        ...
        public static bool JugeMagicDistance(SystemXmlItem systemMagic, IObject attacker, int enemy, int enemyX, int enemyY, int magicCode, bool forceNotAttack = false)
        {
            ...
            const int icCannelDistance = 300;

            //判断是否是单攻
            if (systemMagic.GetIntValue("MagicType") == 1) //单攻
            {
                int targetType = systemMagic.GetIntValue("TargetType");
                if (1 == targetType) //自身后者身前
                {
                    return true;
                }
                else if (2 == targetType || 3 == targetType || 4 == targetType) //敌人或者选中目标, 或者空挥
                {
            ...
            else //群攻
            {
                ...
                //首先根据配置文件算出目标中心点
                if (1 == systemMagic.GetIntValue("TargetPos")) //自身
                {
                    return true;
                }
                else if (2 == systemMagic.GetIntValue("TargetPos")) //锁定目标
                {
        ...
        private static bool CanAutoUseZSSkill(GameClient client, int magicCode)
        {
            ...
            int magicType = systemMagic.GetIntValue("MagicType");

            //单攻和群攻
            return (1 == magicType || 2 == magicType);
        }
        ...
        private static void ParseManyTimes(IObject obj, List<ManyTimeDmageItem> manyTimeDmageItemList, int enemy, int enemyX, int enemyY,  int realEnemyX, int realEnemyY, int magicCode)
        {
            ...
            if (obj is GameClient)
            {
                ...
                if (client.MyMagicsManyTimeDmageQueue.GetManyTimeDmageQueueItemNum() > 0)
                {
                    return;
                }
            ...
            else if (obj is Robot)
            {
                ...
                if (robot.MyMagicsManyTimeDmageQueue.GetManyTimeDmageQueueItemNum() > 0)
                {
                    return;
                }
        ...
        public static void ExecMagicsManyTimeDmageQueue(IObject obj)
        {
            ...
            if (obj is GameClient)
            {
                ...
                {
                    //System.Diagnostics.Debug.WriteLine(string.Format("执行分段技能: MagicCode={0}, Index={1}, Percent={2}, ToExecTicks={3}, Ticks={4}", itemsList[i].magicCode, itemsList[i].manyRangeIndex, itemsList[i].manyRangeInjuredPercent, itemsList[i].ToExecTicks, DateTime.Now.Ticks / 10000));
        ...
        public static void ProcessAttack(GameClient client, int enemy, int enemyX, int enemyY,  int realEnemyX, int realEnemyY, int magicCode, int manyRangeIndex = -1, double manyRangeInjuredPercent = 1.0)
        {
            ...
            //    {
            //        SpriteAttack._ProcessAttack(tcpMgr, socket, tcpClientPool, tcpRandKey, pool, nID, data, count, out tcpOutPacket, client, enemy, enemyX, enemyY, realEnemyX, realEnemyY, Global.ZSAutoUseSkillIDs[i], false);
            //    }
            //}
            ...
            if (-1 == manyRangeIndex) //只有在-1的时候才执行判断
            {
                ...
                if (null != manyTimeDmageItemList && manyTimeDmageItemList.Count > 0)
                {
                    ...
                    // System.Console.WriteLine(string.Format("解析技能{0}：{1}", magicCode, DateTime.Now.Ticks));
            ...
            // System.Console.WriteLine(string.Format("执行技能{0}：{1}", magicCode, DateTime.Now.Ticks));
        ...
        private static void _ProcessAttack(GameClient client, int enemy, int enemyX, int enemyY, int realEnemyX, int realEnemyY, int magicCode, bool recAttackTicks, int manyRangeIndex, double manyRangeInjuredPercent)
        {
            ...
            {
                ProcessPhyAttack(client, enemy, enemyX, enemyY, magicCode, manyRangeIndex, manyRangeInjuredPercent);
            }
            else //带技能的攻击动作
            {
                ProcessMagicAttack(client, enemy, enemyX, enemyY, magicCode, manyRangeIndex, manyRangeInjuredPercent);
            }
        }
        ...
        private static bool IsFriend(GameClient me, GameClient enemy)
        {
            ...
            {
                //非敌对对象
                if (me.ClientData.Faction > 0 && me.ClientData.Faction == enemy.ClientData.Faction)
                {
                    return true;
                }
            }
        ...
        private static void ProcessMagicAttack(GameClient client, int enemy, int enemyX, int enemyY, int magicCode, int manyRangeIndex, double manyRangeInjuredPercent)
        {
            ...
            if (!JugeMagicDistance(systemMagic, client, enemy, enemyX, enemyY, magicCode))
            {
                ...
                if (magicScanTypeItem != null && magicScanTypeItem.MagicActionID != MagicActionIDs.SQUARESCAN &&
            ...
            //判断是否是单攻
            if (systemMagic.GetIntValue("MagicType") == 1 || systemMagic.GetIntValue("MagicType") == 3) //单攻或者自动触发
            {
                int targetType = systemMagic.GetIntValue("TargetType");
                if (1 == targetType) //自身
                {
                    if (systemMagic.GetIntValue("MagicType") != 3)
                    {
                        // 通知所有在线用户某个精灵的被命中(无伤害时才需要单独发送)(同一个地图才需要通知)
                        GameManager.ClientMgr.NotifySpriteHited(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                ...
                else if (-1 == targetType || 2 == targetType || 3 == targetType || 4 == targetType) //敌人或者选中目标
                {
                    ...
                    else if (4 == targetType) //如果是自身或者选中目标
                    {
                        if (-1 == enemy) //如果是空挥, 则默认为自己
                        {
                            enemy = client.ClientData.RoleID;
                    ...
                    else if (-1 == targetType || 3 == targetType) //如果是敌人
                    {
            ...
            else //群攻
            {
                ...
                {
                    if (magicScanTypeItem.MagicActionID == MagicActionIDs.SQUARESCAN)
                    {
                    ...
                    {
                        GameManager.ClientMgr.LookupEnemiesInCircleByAngle(client, client.ClientData.RoleDirection, client.ClientData.MapCode, (int)targetPos.X, (int)targetPos.Y, Global.SafeConvertToInt32(systemMagic.GetStringValue("AttackDistance")), clientList, magicScanTypeItem.MagicActionParams[0], true);
                    }
                ...
                {
                    GameManager.ClientMgr.LookupEnemiesInCircle(client, client.ClientData.MapCode, (int)targetPos.X, (int)targetPos.Y, Global.SafeConvertToInt32(systemMagic.GetStringValue("AttackDistance")), clientList);
                }
                ...
                else if (2 == targetType || 4 == targetType)//自身和队友(备注: 单攻下无法给其他人加魔法)
                {
                    //命中特效的播放类型
                    if (targetPlayingType <= 0)
                    {
                    ...
                    for (int j = 0; j < magicActionItemList.Count; j++)
                    {
                        MagicAction.ProcessAction(client, client, magicActionItemList[j].MagicActionID, magicActionItemList[j].MagicActionParams, (int)targetPos.X, (int)targetPos.Y, subMagicV, skillData.SkillLevel, skillData.SkillID, 0, 0, attackDirection, 0, false, false, manyRangeInjuredPercent);
                    }
                    ...
                    for (int i = 0; i < clientList.Count; i++)
                    {
                        ...
                        for (int j = 0; j < magicActionItemList.Count; j++)
                        {
                            MagicAction.ProcessAction(client, clientList[i] as GameClient, magicActionItemList[j].MagicActionID, magicActionItemList[j].MagicActionParams, (int)targetPos.X, (int)targetPos.Y, subMagicV, skillData.SkillLevel, skillData.SkillID, 0, 0, attackDirection, 0, false, false, manyRangeInjuredPercent);
                        }
                ...
                else if (-1 == targetType || 3 == targetType) //敌人或者选中目标
                {
                    //命中特效的播放类型
                    if (targetPlayingType <= 0)
                    {
                    ...
                    for (int i = 0; i < monsterList.Count; i++)
                    {
                        ...
                        for (int j = 0; j < magicActionItemList.Count; j++)
                        {
                            MagicAction.ProcessAction(client, monsterList[i] as Monster, magicActionItemList[j].MagicActionID, magicActionItemList[j].MagicActionParams, (int)targetPos.X, (int)targetPos.Y, subMagicV, skillData.SkillLevel, skillData.SkillID, 0, 0, attackDirection, 0, false, false, manyRangeInjuredPercent);
                        }
        ...
        public static void ProcessAttackByMonster(Monster attacker, int enemy, int enemyX, int enemyY, int realEnemyX, int realEnemyY, int magicCode, int manyRangeIndex = -1, double manyRangeInjuredPercent = 1.0)
        {
            ...
            {
                List<ManyTimeDmageItem> manyTimeDmageItemList = MagicsManyTimeDmageCachingMgr.GetManyTimeDmageItems(magicCode);
                if (null != manyTimeDmageItemList && manyTimeDmageItemList.Count > 0 && attacker.MagicFinish == -1)
                {
                    //System.Console.WriteLine(string.Format("解析技能{0}：{1}", magicCode, DateTime.Now.Ticks));
            ...
            //System.Console.WriteLine(string.Format("执行技能{0}：{1}", attacker.CurrentMagic, DateTime.Now.Ticks));
        ...
        private static void ProcessMagicAttackByJingJiRobot(Robot attacker, IObject target, int magicCode, int manyRangeIndex, double manyRangeInjuredPercent)
        {
            ...
            if (systemMagic.GetIntValue("MagicType") == 1 || systemMagic.GetIntValue("MagicType") == 3) //单攻或者自动触发
            {
                ...
                else if (-1 == targetType || 2 == targetType || 3 == targetType) //敌人或者选中目标
                {
            ...
            else //群攻
            {
                ...
                else if (2 == targetType) //自身和队友(备注: 单攻下无法给其他人加魔法)
                {
                    //命中特效的播放类型
                    if (targetPlayingType <= 0)
                    {
...
//                     GameManager.MonsterMgr.LookupRangeAttackEnemies(attacker, (int)targetPos.X, (int)targetPos.Y, attackDirection, systemMagic.GetStringValue("AttackDistance"), monsterList);
                    ...
                    if (magicScanTypeItem != null)
                    {
                        ...
                        {
                            GameManager.MonsterMgr.LookupEnemiesInCircleByAngle((int)attacker.Direction, attacker.CurrentMapCode, attacker.CopyMapID, (int)attacker.CurrentPos.X, (int)attacker.CurrentPos.Y, (int)targetPos.X, (int)targetPos.Y, Global.SafeConvertToInt32(systemMagic.GetStringValue("AttackDistance")), clientList, magicScanTypeItem.MagicActionParams[0], true);
                        }
                    }
                    else
                    {
                        GameManager.MonsterMgr.LookupEnemiesInCircle(attacker.CurrentMapCode, attacker.CopyMapID, (int)attacker.CurrentPos.X, (int)attacker.CurrentPos.Y, (int)targetPos.X, (int)targetPos.Y, Global.SafeConvertToInt32(systemMagic.GetStringValue("AttackDistance")), clientList);
                    }
        ...
        private static void _ProcessAttackByMonster(Monster attacker, int enemy, int enemyX, int enemyY, int realEnemyX, int realEnemyY, int magicCode, bool recAttackTicks, int manyRangeIndex, double manyRangeInjuredPercent)
        {
            ...
            {
                ProcessMagicAttackByMonster(attacker, enemy, enemyX, enemyY, magicCode, manyRangeIndex, manyRangeInjuredPercent);
            }
        }
        ...
        /// <param name="enemyY"></param>
        /// <param name="magicCode"></param>
        private static void ProcessMagicAttackByMonster(Monster attacker, int enemy, int enemyX, int enemyY, int magicCode, int manyRangeIndex, double manyRangeInjuredPercent)
        {
            //技能ID不能为空
            if (-1 == magicCode) return;
            ...
            //System.Console.WriteLine(string.Format("bFindTarget{0}:{1}", bFindTarget, DateTime.Now.Ticks));

            if (attacker.MyMagicsManyTimeDmageQueue.GetManyTimeDmageQueueItemNum() < 1 || !bFindTarget)
            {
                //System.Console.WriteLine(string.Format("结束释放技能{0}:{1}", attacker.CurrentMagic, DateTime.Now.Ticks));
