...
namespace GameServer.Logic
{
    ...
    class WorldLevelManager
    {
        ...
        public void ResetWorldLevel()
        {
            ...
            TCPProcessCmdResults dbRequestResult = Global.RequestToDBServer2(Global._TCPManager.tcpClientPool, TCPOutPacketPool.getInstance(), (int)TCPGameServerCmds.CMD_SPR_GETPAIHANGLIST, strcmd, out tcpOutPacket);
            ...
            if (0 != nBakResetWorldLevelDayID)
            {
                ...
                if (null != allClient)
                {
                    for (int i = 0; i < allClient.Count(); i++)
                    {
                        if (null != allClient[i])
                        {
                            UpddateWorldLevelBuff(allClient[i]);
                        }
                    }
                }
            }
