    ...
    class ShareManager
    {
       ...
       public static List<GoodsData> ShareGoodslist
        ...
        public static bool CanGetShareAward(GameClient client)
        {
            ...
            if (olddayid == DateTime.Now.DayOfYear.ToString()&&fields[1]=="0")
            {
                return true;
            }
            return false;
        ...
        public static bool HasDoneShare(GameClient client)
        {
            ...
            string[] fields = oldstr.Split(',');
            string olddayid = fields[0];
            if (olddayid == DateTime.Now.DayOfYear.ToString())
            {
                return true;
            }
            return false;
        ...
        public static int UpdateRoleShareState(GameClient client)
        {
            ...
            if (fields[0] == DateTime.Now.DayOfYear.ToString()) //今天已经分享，不需要更新
                return 1;
            else
                Global.SaveRoleParamsStringToDB(client, RoleParamName.DailyShare,string.Format("{0},{1}", DateTime.Now.DayOfYear,0), true);
            return ret;
        }
                ...
                if (Global.CanAddGoodsDataList(client, ShareGoodslist))
                {
                    ...
                    foreach (var item in goodlist)
                    {
                        ...
                        Global.AddGoodsDBCommand(Global._TCPManager.TcpOutPacketPool, client,
                           item.GoodsID, item.GCount,
                           item.Quality, "", item.Forge_level,
                           item.Binding, 0, "", true, 1,
                           "分享", Global.ConstGoodsEndTime, item.AddPropIndex, item.BornIndex, item.Lucky, item.Strong, item.ExcellenceInfo, item.AppendPropLev, item.ChangeLifeLevForEquip, item.WashProps);
                    }
                    Global.SaveRoleParamsStringToDB(client, RoleParamName.DailyShare, string.Format("{0},{1}", DateTime.Now.DayOfYear, 1), true);
                }
                else
                {
                    ret = -3;//背包已满
