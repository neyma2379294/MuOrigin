...
namespace GameServer.Logic
{
    ...
    public class MonsterZone
    {
        ...
        /// 静态引用的怪物数据
        /// </summary>
        private MonsterStaticInfo MonsterInfo = null;
        ...
        private void LoadMonster(Monster monster, MonsterZone monsterZone, MonsterStaticInfo monsterInfo, int monsterType, int roleID, string name, double life, double mana, Point coordinate, double direction, double moveSpeed, int attackRange)
        {
            ...
            //人为导致搜索的时间间隔错开
            monster.NextSeekEnemyTicks = 3000 + Global.GetRandomNumber(0, 2000);
        }
        ...
        /// <returns></returns>
        private Monster InitMonster(XElement monsterXml, int maxLifeV, int maxMagicV, XElement xmlFrameConfig, /*XElement xmlPictureConfig, */double moveSpeed, /*int[] speedTickList, */bool attachEvent = true)
        {
        ...
        public void LoadStaticMonsterInfo()
        {
            ...
            if (null == xml)
            {
                LogManager.WriteLog(LogTypes.Error, string.Format("加载指定怪物的衣服文件:{0}, {1}, 失败。启用默认XML配置文件!", Global.GetSafeAttributeStr(monsterXml, "SName"), fileName));
                fileName = defaultFileName;
            ...
            else if ((int)MonsterBirthTypes.TimePoint == BirthType) //按照时间点的爆怪机制
            {
                ...
                if (null != BirthTimePointList)
                {
                    ...
                    if (CanBirthOnTimePoint(now, birthTimePoint))
                    {
                        ...
                        //上次爆怪物的天ID
                        LastBirthDayID = DateTime.Now.DayOfYear;
        ...
        public Boolean CanTodayReloadMonsters()
        {
            ...
            DateTime now = DateTime.Now;
            int days2Kaifu = Global.GetDaysSpanNum(now, kaifuTime);
            ...
            for (int i = 0; i < MonsterList.Count; i++)
            {
                ...
                if (!MonsterList[i].Alive) //如果怪物已经死亡
                {
                    if (((int)MonsterBirthTypes.TimeSpan == BirthType || (int)MonsterBirthTypes.CopyMapLike == BirthType) && Timeslot > 0) //如果是按照时间段来刷怪的这里要进行判断 // 增加副本复活时间 [3/3/2014 LiaoWei]
                    {
                        ...
                        if (DateTime.Now.Ticks - MonsterList[i].LastDeadTicks < monsterRealiveTimeslot)
                        {
                            continue;
                        }
                    }
        ...
        public void LoadDynamicMonsters(MonsterZoneQueueItem monsterZoneQueueItem)
        {
            ...
            for (int i = 0; i < monsterZoneQueueItem.BirthCount; i++)
            {
                ...
                else
                {
                    ...
                    {
                        Point pos = new Point(monsterZoneQueueItem.ToX, monsterZoneQueueItem.ToY);
                ...
                // 庆功宴
                GameManager.QingGongYanMgr.OnLoadDynamicMonsters(monster);
                ...
                // 血色堡垒处理  begin [11/11/2013 LiaoWei]
                /*if (Global.IsBloodCastleSceneID(monsterZoneQueueItem.MyMonsterZone.MapCode))
                {
                    monster.MonsterInfo.BloodCastJiFen = monsterZoneQueueItem.seedMonster.MonsterInfo.BloodCastJiFen;
                    GameManager.BloodCastleMgr.AddBloodCastleDynamicMonster(monsterZoneQueueItem.MyMonsterZone.MapCode, monster);  // 血色堡垒里面动态刷出的怪 进行管理
                }*/
                
                // 血色堡垒处理  end [11/11/2013 LiaoWei]

                // 恶魔广场  begin [12/25/2013 LiaoWei]
                /*if (Global.IsDaimonSquareSceneID(monsterZoneQueueItem.MyMonsterZone.MapCode))
                {
                    monster.MonsterInfo.DaimonSquareJiFen = monsterZoneQueueItem.seedMonster.MonsterInfo.DaimonSquareJiFen;
                    DaimonSquareSceneManager.AddDaimonSquareDynamicMonster(monsterZoneQueueItem.MyMonsterZone.MapCode, monster);  // 恶魔广场里面动态刷出的怪 进行管理
                } */

                // 恶魔广场  end [12/25/2013 LiaoWei]

                // 经验副本 [4/4/2014 LiaoWei]
                //if (monsterZoneQueueItem.MyMonsterZone.MapCode == ExperienceCopySceneManager.m_ExperienceListCopyMaps)
               /// {
               //     GameManager.AngelTempleMgr.m_AngelTempleBoss = monster;
               // }
                                                    ...
                                                    double recoverLifeV, double recoverMagicV, double DamageThornPercent, double DamageThorn, double SubAttackInjurePercent, double SubAttackInjure,
                                                        double IgnoreDefensePercent, double IgnoreDefenseRate, double Lucky, double FatalAttack, double DoubleAttack, int fallGoodsPackID, 
                                                            int monsterType, int battlePersonalJiFen, int battleZhenYingJiFen, int nDaimonSquareJiFen, int nBloodCastJiFen, int fallBelongTo,
                                                                int[] skillIDs, int attackType, int camp, int AIID, int nChangeLifeCount)
        {
