...
namespace GameServer.Logic.JingJiChang.FSM
{
    ...
    internal class AttackState : IFSMState
    {
        ...
        private bool testAttackDistance(out int direction)
        {
            ...
            if (systemMagic.GetIntValue("MagicType") == 1 || systemMagic.GetIntValue("MagicType") == 3)
            {
        ...
        private void selectFiveComboSkill()
        {
            if (isSelectFiveComboSkill)
                return;
            if (fiveComboSkillIndex >= fiveComboSkillList.Length)
            {
            ...
            skillId = fiveComboSkillList[fiveComboSkillIndex++];
        ...
        private void selectSkill(out bool isFiveCombo)
        {
            ...
            while (skillId == -1)
            {
                ...
                if (!owner.MyMagicCoolDownMgr.SkillCoolDown(owner.MonsterInfo.SkillIDs[index]))
                    continue;

                if (!SkillNeedMagicVOk(owner.MonsterInfo.SkillIDs[index]))
                    continue;

                skillId = owner.MonsterInfo.SkillIDs[index];
