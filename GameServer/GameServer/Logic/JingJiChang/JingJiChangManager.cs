...
namespace GameServer.Logic.JingJiChang
{
    ...
    public class JingJiChangConstants
    {
        ...
        public static readonly int[][] SkillFrameCounts = new int[][] {
            new int[]{120,11, 5},new int[]{105,15, 9},new int[]{106,11, 6},new int[]{100,2, 2},new int[]{187,2, 2},new int[]{188,2, 2},new int[]{189,2, 2},new int[]{190,7, 4},
            new int[]{204,11, 6},new int[]{206,13, 6},new int[]{220,11, 6},new int[]{200,2, 2},new int[]{287,2, 2},new int[]{288,2, 2},new int[]{289,2, 2},new int[]{290,7, 4},
            new int[]{305,10, 6},new int[]{301,11, 6},new int[]{306,11, 6},new int[]{300,2, 2},new int[]{388,2, 2},new int[]{389,2, 2},new int[]{390,2, 2},new int[]{391,7, 4}
        };
    ...
    public class JingJiChangManager : JingJiChangConstants, IManager
    {
        ...
        public JingJiDetailData getDetailData(GameClient player, int requestType = 0)
        {
            ...
            //获取玩家竞技场数据
            PlayerJingJiData jingjiData = Global.sendToDB<PlayerJingJiData>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_GET_DATA, DataHelper.ObjectToBytes<int>(player.ClientData.RoleID));
...
//                 return detailData;
                ...
                createJingJiData(player);
                jingjiData = Global.sendToDB<PlayerJingJiData>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_GET_DATA, DataHelper.ObjectToBytes<int>(player.ClientData.RoleID));
            }
            ...
            //向DB请求获取被挑战者数据
            List<PlayerJingJiMiniData> beChallengerMiniDatas = Global.sendToDB<List<PlayerJingJiMiniData>>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_GET_CHALLENGE_DATA, DataHelper.ObjectToBytes<int[]>(beChallengerRankings));
        ...
        public void GetNextRewardTime(GameClient player)
        {
            ...
            {
                //从DB获取排名和下次领取时间
                long[] resultParams = Global.sendToDB<long[]>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_GET_RANKING_AND_NEXTREWARDTIME, DataHelper.ObjectToBytes<int>(player.ClientData.RoleID));
        ...
        public void rankingReward(GameClient player, out int result, out long nextRewardTime)
        {
            ...
            lock (player)
            {
                ...
                //更新DB下次领取时间
                Global.sendToDB<int>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_UPDATE_NEXTREWARDTIME, DataHelper.ObjectToBytes<long[]>(new long[] { player.ClientData.RoleID, _nextRewardTime }));
        ...
        private void addGoods(GameClient player, string goodsInfos)
        {
            ...
            foreach(string goodsInfo in _goodsInfos)
            {
                ...
                Global.AddGoodsDBCommand(TCPOutPacketPool.getInstance(), player, goodsId, goodsNum, 0, "", forgeLevel, binding, 0, "", false, 0, "竞技场排行榜奖励", Global.ConstGoodsEndTime,0,0,lucky,0,ExcellenceProperty,nAppendPropLev,0);
            }
        ...
        public int removeCD(GameClient player)
        {
            ...
            //将挑战时间置0
            Global.sendToDB<bool>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_REMOVE_CD, DataHelper.ObjectToBytes<int>(player.ClientData.RoleID));
        ...
        /// </summary>
        /// <param name="player"></param>
        public void createJingJiData(GameClient player)
        {
            ...
            Global.sendToDB<byte>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_CREATE_DATA, DataHelper.ObjectToBytes<PlayerJingJiData>(data));
            ...
            {
                //初始化军衔
                GameManager.ClientMgr.ModifyShengWangLevelValue(player, 0, "初始化军衔一", true, true);
                //MU军衔播报
                Global.BroadcastClientMUShengWang(player, getJunxian(player));
            }
        ...
        private List<PlayerJingJiSkillData> getSaveSkillData(GameClient client)
        {
            ...
            int baseOccupation = Global.CalcOriginalOccupationID(client);
        ...
        private double[] getExtProps(GameClient player)
        {
            ...
            extProps[(int)ExtPropIndexes.MagicRecoverPercent] = RoleAlgorithm.GetMagicRecoverValPercentV(player);      // 魔法恢复(百分比)
            extProps[(int)ExtPropIndexes.LifeRecover] = RoleAlgorithm.GetLifeRecoverValPercentV(player);      // 单位时间恢复的生命恢复(固定值)
            extProps[(int)ExtPropIndexes.MagicRecover] = RoleAlgorithm.GetMagicRecoverAddPercentV(player);      // 单位时间恢复的魔法恢复(固定值)
        ...
        public int requestChallenge(GameClient player, int beChallengerId, int beChallengerRanking, int enterType)
        {
            ...
            //请求挑战数据
            JingJiBeChallengeData requestChallengeData = Global.sendToDB<JingJiBeChallengeData>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_REQUEST_CHALLENGE, DataHelper.ObjectToBytes<int[]>(new int[] { player.ClientData.RoleID, beChallengerId, beChallengerRanking }));
...
#endif
            // 满血满蓝
            GameManager.ClientMgr.UserFullLife(client, false);
        ...
        /// <param name="beChallengerData"></param>
        /// <returns></returns>
        private Robot createRobot(GameClient player, PlayerJingJiData beChallengerData)
        {
            ...
            //最大最小攻击力
            robot.MonsterInfo.MinAttack = baseOccupation == 1 ? (int)beChallengerData.extProps[(int)ExtPropIndexes.MinMAttack] : (int)beChallengerData.extProps[(int)ExtPropIndexes.MinAttack];
            robot.MonsterInfo.MaxAttack = baseOccupation == 1 ? (int)beChallengerData.extProps[(int)ExtPropIndexes.MaxMAttack] : (int)beChallengerData.extProps[(int)ExtPropIndexes.MaxAttack];
            //物防法防
            ...
            robot.MonsterInfo.AttackType = baseOccupation == 1 ? (int)AttackType.MAGIC_ATTACK : (int)AttackType.PHYSICAL_ATTACK;
            //无阵营
            robot.MonsterInfo.Camp = -1;
        ...
        private RoleDataMini createRoleDataMini(int roleId, PlayerJingJiData data)
        {
            ...
            // 增加翅膀数据
            string strCmd = string.Format("{0}", data.roleId);
            RoleData4Selector roleSelectData = Global.sendToDB<RoleData4Selector>((int)TCPGameServerCmds.CMD_SPR_GETROLEUSINGGOODSDATALIST, strCmd);
            if (null != roleSelectData)
            {
                roleData.MyWingData = roleSelectData.MyWingData;
            }
        ...
        {
            if (null == equipDatas) return null;
        ...
        {
            int ranking = getChallengeEndRanking(resultData);
            ...
            int winCount = Global.sendToDB<int>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_SAVE_DATA, DataHelper.ObjectToBytes<JingJiSaveData>(saveData));
        ...
        /// </summary>
        /// <returns></returns>
        private int getChallengeEndRanking(JingJiChallengeResultData resultData)
        {
            return Global.sendToDB<int>((int)TCPGameServerCmds.CMD_DB_JINGJICHANG_CHALLENGE_END, DataHelper.ObjectToBytes<JingJiChallengeResultData>(resultData));
        }
        ...
        public void onJingJiFuBenStarted(JingJiChangInstance instance)
        {
            ...
            //AI进入竞技场
            GameManager.MonsterZoneMgr.AddDynamicRobot(player.CurrentMapCode, robot, player.ClientData.CopyMapID, 1, gridX, gridY, 1);

            //通知客户端
            SendMySelfJingJiFakeRoleItem(player,robot);
            //开始战斗
            robot.startAttack();
        }
