...
namespace GameServer.Logic
{
    ...
    public class CGetOldResourceManager
    {
        ...
        private static void CalcOldResourceInfo(int oldday, int oldnum, int total, CGetResData data, OldResourceInfo inInfo, out OldResourceInfo outInfo)
        {
            outInfo = inInfo;
            int leftnum = 0;
            int yesterdayid = DateTime.Now.DayOfYear - 1;
        ...
        private static void GetFubenResourceInfo(GameClient client, int copyId, int total, CGetResData data, OldResourceInfo inInfo, out OldResourceInfo outInfo)
        {
            outInfo = inInfo;
            if (total < 1)
                return;
           ...
           // OldResourceInfo tempInfo = new OldResourceInfo();
            foreach (var data in datalist)
            {
                ...
                if (data.copyId != -1)
                {
                    ...
                    if(data.type==3||data.type==4)//经验副本和金币副本
                        total = systemFuBenItem.GetIntValue("EnterNumber");
                    OldResourceInfo tempdata = null;
                    GetFubenResourceInfo(client, data.copyId, total, data, tempdata, out tempdata);
                    if (tempdata != null)
                    {
                ...
                if(data.activeId!=-1)
                {
                    ...
                    switch ((CandoType)data.type)
                    {
                        ...
                        case CandoType.DemonSquare:
                            {
                                ...
                                int nCount = Global.QueryDayActivityEnterCountToDB(client, client.ClientData.RoleID, nDate - 1, (int)SpecialActivityTypes.DemoSque);
                                if (nCount < 0)
                                    nCount = 0;
                        ...
                        case CandoType.BloodCity:
                            {
                                ...
                                int nDate = DateTime.Now.DayOfYear;                // 当前时间
                                int nCount = Global.QueryDayActivityEnterCountToDB(client, client.ClientData.RoleID, nDate - 1, (int)SpecialActivityTypes.BloodCastle);
                                if (nCount < 0)
                                    nCount = 0;
                                leftnum = bcDataTmp.MaxEnterNum + nNum - nCount;
                            }
                        ...
                        case CandoType.AngelTemple:
                            {
                                ...
                                int nDate = DateTime.Now.DayOfYear;                // 当前时间
                                int nCount = Global.QueryDayActivityEnterCountToDB(client, client.ClientData.RoleID, nDate - 1, (int)SpecialActivityTypes.AngelTemple);
                                if (nCount < 0)
                                    nCount = 0;
                        ...
                        case CandoType.PartWar:
                            {
                                ...
                                int nDate = DateTime.Now.DayOfYear;                // 当前时间
                                int nCount = Global.QueryDayActivityEnterCountToDB(client, client.ClientData.RoleID, nDate - 1, (int)SpecialActivityTypes.CampBattle);
                                if (nCount < 0)
                                    nCount = 0;
                                if (fields != null)
                        ...
                        case CandoType.PKKing:
                            {
                                ...
                                int nCount = Global.QueryDayActivityEnterCountToDB(client, client.ClientData.RoleID, nDate - 1, (int)SpecialActivityTypes.TheKingOfPK);
                                if (nCount < 0)
                                    nCount = 0;
                                if (fields != null)
                                    leftnum = fields.Length - nCount;
                        ...
                        case CandoType.OldBattlefield://古战场昨天进入数据
                            {
                                int nDate = DateTime.Now.DayOfYear;   
                                int nCount = Global.QueryDayActivityEnterCountToDB(client, client.ClientData.RoleID, nDate - 1, (int)SpecialActivityTypes.OldBattlefield);
                                if (nCount < 0)
                                    nCount = 0;
                                leftnum = 1 - nCount;
                            }
                ...
                if (data.type == (int)CandoType.Arena)
                {
                   ...
                   GetFubenResourceInfo(client, jingjiFuBenId, total, data, outInfo, out outInfo);
        ...
        public static void InitRoleOldResourceInfo(GameClient client, bool isFirstLogin)
        {
            ...
            //计算出可找回的资源信息
            //List<OldResourceInfo> candolist = new List<OldResourceInfo>();
            ...
            for (int i = 1; i <= (int)CandoType.MaxCandoTypeNum; i++)
            {
                ...
                ComputeResourceByType(client, i, getRestDataDict, out info);
                if (info != null)
                {
                    ResourceInfoDict[i] = info;
                    ResourceInfoDict[i].roleId = client.ClientData.RoleID;
                    
                }
                    
                //else
                //{
                //    info = new OldResourceInfo()
                //    {
        ...
        public static int GiveRoleOldResource(GameClient client, int actType, int goldorZuanshi, int getModel)
        {
            ...
            else
            {
                ...
                for (int i = 1; i < (int)CandoType.MaxCandoTypeNum; i++)
                {
                    if (!client.ClientData.OldResourceInfoDict.TryGetValue(i, out dataInfo))
                    {
                        continue;
                    }
                    if (dataInfo.leftCount == 0)
            ...
            switch (goldorZuanshi)
            {
                case 0:
                    {
                        if (cost > client.ClientData.Money1 + client.ClientData.Money2)
                            return -1;
                        if (Global.SubBindTongQianAndTongQian(client, cost, "资源找回"))
                        {
                            if (dataToclient.exp > 0)
                            {
                                ...
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client,
                                                                            StringUtil.substitute(Global.GetLang("恭喜获得经验 +" + dataToclient.exp)),
                            ...
                            if (dataToclient.bandmoney > 0)
                            {
                                ...
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client,
                                                                               StringUtil.substitute(Global.GetLang("恭喜获得绑定金币 +" + givemoney)),
                ...
                case 1:
                    {
                        ...
                        if (GameManager.ClientMgr.SubUserMoney(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool, client, cost, "资源找回"))
                        {
                            ...
                            if (dataToclient.bandmoney > 0)
                            {
                                ...
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client,
                                                                               StringUtil.substitute(Global.GetLang("恭喜获得绑定金币 +" + dataToclient.bandmoney)),
                                                                               GameInfoTypeIndexes.Hot, ShowGameInfoTypes.OnlyErr, (int)HintErrCodeTypes.None);
                            }
            ...
            lock (client)
            {
                ...
                else
                {
                    ...
                    //    client.ClientData.OldResourceInfoDict[i] = new OldResourceInfo()
                    //    {
                    //        roleId = client.ClientData.RoleID
                    //    };
                    //}
        ...
        public static void ReplaceDataToDB(GameClient client)
        {
            ...
            Global.sendToDB<int>((int)TCPGameServerCmds.CMD_DB_UPDATE_OLDRESOURCE, DataHelper.ObjectToBytes<Dictionary<int, Dictionary<int, OldResourceInfo>>>(dict));
        }
        ...
        public static Dictionary<int, OldResourceInfo> ReadResourceGetfromDB(GameClient client)
        {
            ...
            if (TCPProcessCmdResults.RESULT_FAILED == Global.RequestToDBServer3(Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool,
                (int)TCPGameServerCmds.CMD_DB_QUERY_GETOLDRESINFO, string.Format("{0}", client.ClientData.RoleID), out bytesData))
            {
                return dict; //查询失败
            }
            ...
            if (dict == null)
            {
                dict = new Dictionary<int, OldResourceInfo>();
                for (int i = 1; i < (int)CandoType.MaxCandoTypeNum; i++)
                {

                    dict[i] = new OldResourceInfo()
                    {
                        roleId = client.ClientData.RoleID
                    };
                }
