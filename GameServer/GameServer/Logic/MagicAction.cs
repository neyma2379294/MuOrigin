        ...
        public static bool ProcessAction(IObject self, IObject obj, MagicActionIDs id, double[] actionParams, int targetX = -1, int targetY = -1, int usedMaigcV = 0, int skillLevel = 1, int skillid = -1, int npcID = 0, int binding = 0, int direction = -1, int actionGoodsID = 0, bool bItemAddVal = false, bool bIsVerify = false, double manyRangeInjuredPercent = 1.0) // 增加技能等级参数 [3/13/2014 LiaoWei]
        {
            ...
            switch (id)
            {
                ...
                case MagicActionIDs.NEW_MAGIC_SUBINJURE: //给释放者增加一个魔法护盾，吸收X比例伤害，1级技能持续时间,1级吸收伤害比列,2级技能持续时间,2级吸收伤害比列,3级技能持续时间,3级吸收伤害比列
                    {
                        if (self is GameClient)
                        {
                            ...
                            (self as GameClient).ClientData.FSHuDunStart = DateTime.Now.Ticks / 10000;
                ...
                case MagicActionIDs.DS_HIDE_ROLE: //隐身	持续时间(秒)						
                    {
                        ...
                        /*if (self is GameClient)
                        {
                            if (obj is GameClient)
                            {
                                ...
                                (obj as GameClient).ClientData.DSHideStart = DateTime.Now.Ticks / 10000 + (((long)actionParams[0] + injure) * 1000);
                ...
                case MagicActionIDs.TIME_DS_ADD_DEFENSE: //1级持续时间,1级加成物理防御值,2级持续时间,2级加成物理防御值,3级持续时间,3级加成物理防御值
                    {
                        if (self is GameClient)
                        {
                            ...
                            long ticks = DateTime.Now.Ticks + (((long)actionParams[2 * skillLevel] + injure) * 1000 * 10000);
                            double addValue = actionParams[skillLevel * 2 + 1];
                ...
                case MagicActionIDs.TIME_DS_SUB_DEFENSE: //1级持续时间,1级降低防御,2级持续时间,2级降低防御,3级持续时间,2级降低防御
                    {
                        if (self is GameClient)
                        {
                            ...
                            long ticks = DateTime.Now.Ticks + ((long)actionParams[skillLevel * 2] * 1000 * 10000);
                ...
                case MagicActionIDs.TIME_DS_INJURE: //伤害时间间隔,1级持续时间,1级伤害比列,2级持续时间,2级伤害比列,3级持续时间,3级伤害比列
                    {
                        ...
                        /*if (self is GameClient)
                        {
                            ...
                            if (obj is GameClient)
                            {
                                ...
                                (obj as GameClient).ClientData.ZhongDuStart = DateTime.Now.Ticks / 10000;
                ...
                case MagicActionIDs.INSTANT_ATTACK2LIFE: //直接物理伤害 + 多少值, 并将多少百分比的伤害转换为自己的血量	要增加的物理伤害值	转换伤害的百分比 *
                    {
                        if (self is GameClient) //发起攻击者是角色
                        {
                            ...
                            if (injured > 0)
                            {
                                ...
                                GameManager.ClientMgr.AddSpriteLifeV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, self as GameClient, addLife);
                            }
                            else if (obj is FakeRoleItem) //被攻击者是假人
                            {
                                FakeRoleManager.NotifyInjured(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                ...
                case MagicActionIDs.TIME_ADDATTACK:	//持续增加物理攻击力	增加百分比	持续多长时间
                    {
                        long ticks = DateTime.Now.Ticks + ((long)actionParams[1] * 1000 * 10000);
                ...
                case MagicActionIDs.TIME_SUBMAGIC:	//持续降低魔法攻击力	降低百分比	持续多长时间
                    ...
                    break;
                case MagicActionIDs.INSTANT_ADDLIFE1:	//直接加血	加的血量
                    {
                        if (obj is GameClient) //如果是角色
                        {
                            GameManager.ClientMgr.AddSpriteLifeV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                obj as GameClient, actionParams[0]);
                        }
                    }
                    break;
                case MagicActionIDs.INSTANT_ADDMAGIC1:	//直接加魔	加的魔量
                    {
                        if (obj is GameClient) //如果是角色
                        {
                            if (obj is GameClient) //如果是角色
                            {
                                GameManager.ClientMgr.AddSpriteMagicV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                    obj as GameClient, actionParams[0]);
                            }
                        }
                    }
                ...
                case MagicActionIDs.GOTO_MAPBYYUANBAO:	//回某个地图的固定的位置	地图编号
                    {
                        ...
                        if (self is GameClient)
                        {
                            ...
                            if (needYuanBao > 0)
                            {
                                subOk = GameManager.ClientMgr.SubUserMoney(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool,
                                    Global._TCPManager.TcpOutPacketPool, self as GameClient, needYuanBao, "GOTO_MAPBYYUANBAO公式");
                ...
                case MagicActionIDs.GOTO_LAST_MAP:	//回上一个地图的最后位置
                    {
                        ...
                        if ((self as GameClient).ClientData.LastMapCode != -1 && (self as GameClient).ClientData.LastPosX != -1 && (self as GameClient).ClientData.LastPosY != -1
                            && SceneUIClasses.Normal == Global.GetMapSceneType((self as GameClient).ClientData.LastMapCode)
                            )
                        {
                            if (MapTypes.Normal == Global.GetMapType((self as GameClient).ClientData.LastMapCode))
                            {
                                if (GameManager.BattleMgr.BattleMapCode != (self as GameClient).ClientData.LastMapCode
                                    && GameManager.ArenaBattleMgr.BattleMapCode != (self as GameClient).ClientData.LastMapCode
                                    )
                                {
                                    toMapCode = (self as GameClient).ClientData.LastMapCode;
                                    toPosX = (self as GameClient).ClientData.LastPosX;
                                    toPosY = (self as GameClient).ClientData.LastPosY;
                                }
                            }
                        }
                        ...
                        if (GameManager.MapMgr.DictMaps.TryGetValue(toMapCode, out gameMap)) //确认地图编号是否有效
                        {
                            ...
                            {
                                if (Global.IsDaimonSquareSceneID((self as GameClient).CurrentMapCode) && toMapCode != (self as GameClient).CurrentMapCode && (self as GameClient).ClientData.DaimonSquarePoint > 0)
                                {
                                ...
                                else if (Global.IsBloodCastleSceneID((self as GameClient).CurrentMapCode) && toMapCode != (self as GameClient).CurrentMapCode && (self as GameClient).ClientData.BloodCastleAwardPoint > 0)
                                {
                ...
                case MagicActionIDs.NEW_ADD_MONEY:	//增加金币	增加的金币的数量
                    {
                        if (obj is GameClient) //如果是角色
                        {
                            GameManager.ClientMgr.AddMoney1(Global._TCPManager.MySocketListener,
                                Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool,
                                obj as GameClient, (int)actionParams[0], "脚本添加绑金");
                        }
                    }
                    break;
                ...
                case MagicActionIDs.NEW_ADD_YINLIANG:	//增加银两	增加的银两的值
                    {
                        if (obj is GameClient) //如果是角色
                        {
                            GameManager.ClientMgr.AddUserYinLiang(Global._TCPManager.MySocketListener,
                                Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool,
                                obj as GameClient, (int)actionParams[0], "脚本增加金币一");
                        }
                    }
                    break;
                ...
                case MagicActionIDs.NEW_TIME_ATTACK3: //持续物理伤害	原始物理攻击力要乘以的系数值(浮点数)	每增加一级，增加的物理攻击力系数值(浮点数)	持续多长时间	总共几次
                    {
                        if (self is GameClient) //发起攻击者是角色
                        {
                            ...
                            int nDoubleValue = (int)RoleAlgorithm.GetDoubleAttack(self as GameClient);  // 属性改造 双倍一击 [8/15/2013 LiaoWei]
                            int attackV = (int)RoleAlgorithm.CalcAttackValue(self as GameClient, minAttackV, maxAttackV, lucky, nFatalValue, nDoubleValue, out nDamageType);
                ...
                case MagicActionIDs.NEW_TIME_MAGIC3: //持续魔法伤害	原始魔法攻击力要乘以的系数值(浮点数)	每增加一级，增加的魔法攻击力系数值(浮点数)	持续多长时间	总共几次
                    {
                        if (self is GameClient) //发起攻击者是角色
                        {
                            ...
                            int nFatalValue = (int)RoleAlgorithm.GetFatalAttack(self as GameClient);    // 属性改造 卓越一击 [8/15/2013 LiaoWei]
                            int nDoubleValue = (int)RoleAlgorithm.GetDoubleAttack(self as GameClient);  // 属性改造 双倍一击 [8/15/2013 LiaoWei]
                            int magicAttackV = (int)RoleAlgorithm.CalcAttackValue(self as GameClient, minMAttackV, maxMAttackV, lucky, nFatalValue, nDoubleValue, out nDamageType);
                ...
                case MagicActionIDs.NEW_INSTANT_ADDLIFE3: //直接加血	原始魔法攻击力要乘以的系数值(浮点数)	每增加一级，增加的魔法攻击力系数值(浮点数)	
                    {
                        if (self is GameClient) //发起攻击者是角色
                        {
                            if (obj is GameClient) //如果是角色
                            {
                                ...
                                GameManager.ClientMgr.AddSpriteLifeV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                    obj as GameClient, magicAttackV);
                            }
                        }
                    }
                    break;
                ...
                case MagicActionIDs.GOTO_WUXING_MAP: //五行奇阵的传送
                    {
                        ...
                        int needGoodsID = WuXingMapMgr.GetNeedGoodsIDByNPCID((self as GameClient).ClientData.MapCode, (npcID - 0x60000000));
                        if (-1 != needGoodsID)
                        {
                            if (Global.GetTotalGoodsCountByID((self as GameClient), needGoodsID) > 0)
                            {
                                ...
                                {
                                    int gotToMapCode = WuXingMapMgr.GetNextMapCodeByNPCID((self as GameClient).ClientData.MapCode, (npcID - 0x60000000));
                                    if (-1 != gotToMapCode)
                                    {
                                        GameMap gameMap = null;
                ...
                case MagicActionIDs.NEW_JIUHUA_ADDLIFE: //服后可迅速将人物生命值恢复至100%	
                    {
                        ...
                        {
                            GameManager.ClientMgr.AddSpriteLifeV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                obj as GameClient, (obj as GameClient).ClientData.LifeV - (obj as GameClient).ClientData.CurrentLifeV);
                        }
                    }
                    break;
            ...
            case MagicActionIDs.GOTO_ARENABATTLEMAP:	//进入竞技场
                    {
                        ...
                        if (self is GameClient)
                        {
                            Global.ClientEnterArenaBattle((self as GameClient));
                        }
                    }
                    break;
            ...
            case MagicActionIDs.DB_INSTANT_LIFE_MAGIC: //瞬间回复生命值和魔法值的药品公式	生命值	魔法值
                        if (self is GameClient)
                        {
                            ...
                            GameManager.ClientMgr.AddSpriteLifeV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                self as GameClient, addLiefV);

                            GameManager.ClientMgr.AddSpriteMagicV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                self as GameClient, addMagicV);
                        }
                        break;
            ...
            case MagicActionIDs.ADD_BOSSCOPYENTERNUM:	//增加boss副本进入次数 次数
                        if (self is GameClient)
                        {
                            ...
                            if (addNum >= 0)
                            {
                                ...
                                /// 执行npc脚本对话
                                Global.ExecNpcTalkText(client, 2, 0x60000000 + 209, 209, 1);
                            }
                        }
                        break;
            ...
            case MagicActionIDs.MU_ADD_PHYSICAL_ATTACK:     // 增加物理攻击(最小值，最大值，每级值增加的步长)
                ...
                case MagicActionIDs.MU_ADD_HP:
                    {
                        if (self is GameClient)
                        {
                            ...
                            {
                                GameManager.ClientMgr.AddSpriteLifeV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                    obj as GameClient, (int)dResume);
                            }
                            else
                            {
                                GameManager.ClientMgr.AddSpriteLifeV(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                    self as GameClient, (int)dResume);
                            }
                        }
                    }
                    break;
                ...
                case MagicActionIDs.MU_ADD_ATTACK_DOWN:             // 有概率使对方攻击下降30%,持续2秒 (触发概率,下降比例,持续时间)
                    {
                        if (self is GameClient)
                        {
                            ...
                            // 通过BUFFER来实现
                            long ToTick = DateTime.Now.Ticks + ((long)dTime * 1000 * 10000);    // 时长
                            ...
                            if ((obj as GameClient) != null)
                            {
                                ...
                                GameManager.ClientMgr.NotifyRoleStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (obj as GameClient),
                                    (int)RoleStatusIDs.AttackDown, DateTime.Now.Ticks / 10000, (int)dTime);
                            ...
                            else if (obj is Monster)
                            {
                                ...
                                if ((int)MonsterTypes.Noraml == monster.MonsterType || (int)MonsterTypes.Rarity == monster.MonsterType)
                                {
                ...
                case MagicActionIDs.MU_ADD_HUNMI:             // 有概率使对方昏迷,持续2秒(触发概率,持续时间)
                    {
                        ...
                        if (obj is GameClient)
                        {
                            if ((obj as GameClient) != null)
                            {
                                ...
                                //借用冻结的字段
                                (obj as GameClient).ClientData.DongJieStart = DateTime.Now.Ticks / 10000;
                                ...
                                GameManager.ClientMgr.NotifyRoleStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (obj as GameClient),
                                    (int)RoleStatusIDs.Faint, DateTime.Now.Ticks / 10000, (int)dTime, moveCost);
                        ...
                        {
                            Monster monster = (obj as Monster);
                            if ((int)MonsterTypes.Noraml == monster.MonsterType || (int)MonsterTypes.Rarity == monster.MonsterType || (int)MonsterTypes.JUSTMOVE == monster.MonsterType)
                            {
                                monster.DongJieStart = DateTime.Now.Ticks / 10000;
                                monster.DongJieSeconds = (int)dTime;

                                GameManager.ClientMgr.NotifyMonsterStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, monster,
                                    (int)RoleStatusIDs.Faint, DateTime.Now.Ticks / 10000, (int)dTime);
                            }
                        }
                    }
                    break;
                case MagicActionIDs.MU_ADD_DINGSHENG:       // 有概率使对方速度降低100%,持续3秒 (触发概率,减少比例,持续时间)
                    {
                        ...
                        dRate = StateRate.GetNegativeRate(self, obj, dRate, ExtPropIndexes.StateDingSheng);
                        ...
                        if (obj is GameClient)
                        {
                            if ((obj as GameClient) != null)
                            {
                                (obj as GameClient).RoleBuffer.AddTempExtProp((int)ExtPropIndexes.StateDingSheng, 1, ToTick);
                                ...
                                GameManager.ClientMgr.NotifyRoleStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (obj as GameClient),
                                    (int)RoleStatusIDs.DingShen, DateTime.Now.Ticks / 10000, (int)dTime, moveCost);
                            }
                        }
                        else if (obj is Monster)
                        {
                            ...
                            if ((int)MonsterTypes.Noraml == monster.MonsterType || (int)MonsterTypes.Rarity == monster.MonsterType || (int)MonsterTypes.JUSTMOVE == monster.MonsterType)
                            {
                                ...
                                GameManager.ClientMgr.NotifyMonsterStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, monster,
                                    (int)RoleStatusIDs.DingShen, DateTime.Now.Ticks / 10000, (int)dTime, monster.MoveSpeed);
                            }
                        }
                    }
                    break;
                case MagicActionIDs.MU_ADD_MOVESPEED_DOWN:       // 有概率使对方移动速度减少30%,持续3秒 (触发概率,减少比例,持续时间)
                    {
                        ...
                        if (obj is GameClient)
                        {
                            if ((obj as GameClient) != null)
                            {
                                ...
                                GameManager.ClientMgr.NotifyRoleStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (obj as GameClient),
                                    (int)RoleStatusIDs.SlowDown, DateTime.Now.Ticks / 10000, (int)dTime, moveCost);
                            }
                        }
                    }
                    break;
                ...
                case MagicActionIDs.MU_ADD_HIT_DOWN:       // 有概率使对方命中下降30%,持续2秒 (触发概率,下降比例,持续时间)
                    {
                        ...
                        if ((obj as GameClient) != null)
                        {
                            ...
                            GameManager.ClientMgr.NotifyRoleStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (obj as GameClient),
                                (int)RoleStatusIDs.HitDown, DateTime.Now.Ticks / 10000, (int)dTime);
                ...
                case MagicActionIDs.MU_ADD_DEFENSE_DOWN:       // 一段时间内减少防御百分比 (触发概率, 下降比例, 持续时间)
                    {
                        if (self is GameClient)
                        {
                            ...
                            if ((obj as GameClient) != null)
                            {
                                ...
                                GameManager.ClientMgr.NotifyRoleStatusCmd(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (obj as GameClient),
                                    (int)RoleStatusIDs.DefenseDown, DateTime.Now.Ticks / 10000, (int)dTime);   
                            }
                            else if (obj is Monster)
                            {
                                Monster monster = (obj as Monster);
                ...
                case MagicActionIDs.MU_ADD_JITUI:       // 击飞 (触发概率,距离值)
                    ...
                    break;
                // 全新出炉的MU技能 Een [3/14/2014 LiaoWei]
                case MagicActionIDs.GOTO_BLOODCASTLE:       // 进入血色堡垒 -- 各种判断[11/6/2013 LiaoWei]
                    {
                        if (self is GameClient)
                        {
                            GameClient client = self as GameClient;
                            ...
                            if (client.ClientData.BloodCastleAwardPoint > 0)
                            {
                                ...
                                if (Data.BloodCastleDataInfoList.TryGetValue(nSceneID, out bcDataTmp1))
                                {
                                    ...
                                    //int nRemainTime = Global.GetRoleParamsInt32FromDB(client, RoleParamName.BloodCastleSceneTimer);
                                    //int nTimeAward = (int)(bcDataTmp1.TimeModulus * nRemainTime);

                                    // 1.离场倒计时开始 2.是否成功完成 3.玩家的积分 4.玩家经验奖励 5.玩家的金钱奖励 6.玩家物品奖励1(只有提交大天使武器的玩家才有 其他人为null) 7.玩家物品奖励2(通用奖励 大家都有的)
                                    strcmd = string.Format("{0}:{1}:{2}:{3}:{4}:{5}:{6}", -1, nFlag, client.ClientData.BloodCastleAwardPoint,
                                                            client.ClientData.BloodCastleAwardPoint * bcDataTmp1.ExpModulus,
                            ...
                            BloodCastleScene bcTmp = BloodCastleManager.GetBloodCastleListScenes(nMapID);
                            ...
                            // 时限段判断
                            if (!Global.CanEnterBloodCastleOnTime(bcDataTmp.BeginTime, bcDataTmp.PrepareTime))
                            {
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                                    StringUtil.substitute(Global.GetLang("当前时间段血色堡垒并未开启，请稍后再试")), GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, (int)HintErrCodeTypes.None);
                                break;
                            }
                            ...
                            {
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,(self as GameClient), 
                                    StringUtil.substitute(Global.GetLang("血色堡垒大门已关闭，请等待下次活动开启")), GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, (int)HintErrCodeTypes.None);
                                break;
                            }
#if true
                            // 场景人数限制判断
                            if (Interlocked.Increment(ref bcTmp.m_nPlarerCount) > bcDataTmp.MaxPlayerNum)
                            {
                                Interlocked.Decrement(ref bcTmp.m_nPlarerCount);
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                                    StringUtil.substitute(Global.GetLang("血色堡垒玩家数量已达上限，请等待下次活动开启")), GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, (int)HintErrCodeTypes.None);
                                break;
                            }
#else
                            if (bcTmp.m_nPlarerCount >= bcDataTmp.MaxPlayerNum)
                            {
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                                    StringUtil.substitute(Global.GetLang("血色堡垒玩家数量已达上限，请等待下次活动开启")), GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, (int)HintErrCodeTypes.None);
                                break;
...
#endif
                            // 进入次数判断  把进入次数放在t_dayactivityinfo 向DB请求
                            ...
                            else if (nCount >= bcDataTmp.MaxEnterNum)
                            {
                                bool nRet = true;
                            ...
                            // 需要物品判断
                            GoodsData goodData = Global.GetGoodsByID(client, bcDataTmp.NeedGoodsID);
                            if (goodData == null)
                            {
                                Interlocked.Decrement(ref bcTmp.m_nPlarerCount);
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                            ...
                            if (goodData.GCount < bcDataTmp.NeedGoodsNum)
                            {
                                Interlocked.Decrement(ref bcTmp.m_nPlarerCount);
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                            ...
                            if (!GameManager.ClientMgr.NotifyUseGoods(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool, client, bcDataTmp.NeedGoodsID, 1, false, out usedBinding, out usedTimeLimited))
                            {
                                Interlocked.Decrement(ref bcTmp.m_nPlarerCount);
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                            ...
                            // 进入血色堡垒...
                            client.ClientData.bIsInBloodCastleMap = true;
                            GameManager.ClientMgr.NotifyChangeMap(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client, nMapID, -1, -1, -1);
                            ...
                            strcmd = string.Format("{0}:{1}:{2}:{3}:{4}:{5}:{6}:{7}", nMapID, nTimer, bcDataTmp.NeedKillMonster1Num, 1, bcDataTmp.NeedKillMonster2Num, 1, 1, 1);
                            ...
                            GameManager.ClientMgr.SendToClient(client, strcmd, (int)TCPGameServerCmds.CMD_SPR_BLOODCASTLEPLAYERNUMNOTIFY);

                            GameManager.ClientMgr.NotifyBloodCastleMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                                                        bcTmp.m_nMapCode, (int)TCPGameServerCmds.CMD_SPR_BLOODCASTLEPLAYERNUMNOTIFY,0, 0, 0, bcTmp.m_nPlarerCount);

                            // 每日活跃
                            DailyActiveManager.ProcessCompleteDailyActivityForDailyActive(client, (int)SpecialActivityTypes.BloodCastle);

                            // 存住场景ID
                            Global.SaveRoleParamsInt32ValueToDB(client, RoleParamName.BloodCastleSceneid, nMapID, true);
                            Global.SaveRoleParamsInt32ValueToDB(client, RoleParamName.BloodCastleSceneFinishFlag, 0, true);

                            // 记条LOG
                            LogManager.WriteLog(LogTypes.Info, string.Format("{0} enter BloodCastle count={1} time={2}", client.ClientData.RoleID, nCount, DateTime.Now.ToLongDateString()));
                        }

                    }
                    break;
                case MagicActionIDs.GET_AWARD_BLOODCASTLE:      // 领取血色堡垒奖励
                    {
                        if (self is GameClient)
                        {
                            GameClient client = self as GameClient;

                            int nMapID = client.ClientData.MapCode;
                            ...
                            if (null != sItem && sItem.Length > 0)
                            {
                                for (int i = 0; i < sItem.Length; i++)
                                {
                                    ...
                                    string sMsg = "血色堡垒奖励--提交任务者奖励";

                                    if (!Global.CanAddGoodsNum(client, nGoodsNum))
                                    {
                                        for (int j = 0; j < nGoodsNum; ++j)
                                            Global.UseMailGivePlayerAward(client, goods, "血色堡垒奖励", sMsg);
                                    }
                                    else
                            ...
                            // 完成该血色堡垒
                            GameManager.BloodCastleMgr.CompleteBloodCastScene(client, bcTmp, bcDataTmp);
                ...
                case MagicActionIDs.GET_AWARD_BLOODCASTLECOPYSCENE:      // 领取血色堡垒奖励
                    ...
                    break;
                case MagicActionIDs.GOTO_DAIMONSQUARE:       // 进入恶魔广场-- 各种判断 [12/25/2013 LiaoWei]
                    {
                        if (self is GameClient)
                        {
                            GameClient client = self as GameClient;
                            ...
                            if (client.ClientData.DaimonSquarePoint > 0)
                            {
                                ...
                                DaimonSquareDataInfo bcDataTmp1 = null;
                                //bcDataTmp1 = Data.DaimonSquareDataInfoList[nSceneID];

                                if (Data.DaimonSquareDataInfoList.TryGetValue(nSceneID, out bcDataTmp1))
                                {
                                    if (bcDataTmp1 == null)
                                        break;
                                    ...
                                    for (int n = 0; n < bcDataTmp1.AwardItem.Length; ++n)
                                    {
                                        sAwardItem += bcDataTmp1.AwardItem[n];
                                        if (n != bcDataTmp1.AwardItem.Length - 1)
                                            sAwardItem += "|";
                                    }
                                    ...
                                    //int nRemainTime = Global.GetRoleParamsInt32FromDB(client, RoleParamName.DaimonSquareSceneTimer);
                                    //int nTimeAward = (int)(bcDataTmp1.TimeModulus * nRemainTime);

                                    // 1.是否成功完成 2.玩家的积分 3.玩家经验奖励 4.玩家的金钱奖励 5.玩家物品奖励
                                    strcmd = string.Format("{0}:{1}:{2}:{3}:{4}", nFlag, client.ClientData.DaimonSquarePoint, client.ClientData.DaimonSquarePoint * bcDataTmp1.ExpModulus,
                                                            client.ClientData.DaimonSquarePoint * bcDataTmp1.MoneyModulus, sAwardItem);
                            ...
                            int nMapID = Global.GetDaimonSquareSceneIDForRole(client);

                            if (nMapID <= 0)
                            {
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                                    StringUtil.substitute(Global.GetLang("对不起 恶魔广场场景ID错误！！")), GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, (int)HintErrCodeTypes.None);
                                LogManager.WriteLog(LogTypes.Error, string.Format("enter bloodcastle scene fail!! get scene info fail!!!!"));
                                break;
                            }
                            ...
                            if (!Data.DaimonSquareDataInfoList.TryGetValue(nMapID, out bcDataTmp))
                                break;

                            DaimonSquareScene bcTmp = DaimonSquareSceneManager.GetDaimonSquareListScenes(nMapID);
                            ...
                            // 时限段判断
                            if (!Global.CanEnterDaimonSquareOnTime(bcDataTmp.BeginTime, bcDataTmp.PrepareTime))
                            {
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                                    StringUtil.substitute(Global.GetLang("当前时间段恶魔广场并未开启，请稍后再试")), GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, (int)HintErrCodeTypes.None);
                                break;
                            }
                            ...
                            {
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                                    StringUtil.substitute(Global.GetLang("恶魔广场大门已关闭，请等待下次活动开启")), GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, (int)HintErrCodeTypes.None);
                                break;
                            }
                            ...
                            if(Interlocked.Increment(ref bcTmp.m_nPlarerCount) > bcDataTmp.MaxPlayerNum)
                            {
                                Interlocked.Decrement(ref bcTmp.m_nPlarerCount);
                                GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, (self as GameClient),
                                    StringUtil.substitute(Global.GetLang("恶魔广场玩家数量已达上限，请等待下次活动开启")), GameInfoTypeIndexes.Error, ShowGameInfoTypes.ErrAndBox, (int)HintErrCodeTypes.None);
                                break;
                            }
#else
                            if (bcTmp.m_nPlarerCount >= bcDataTmp.MaxPlayerNum)
                            ...
                            int nType = (int)SpecialActivityTypes.DemoSque;
                            ...
                            // 进入恶魔广场...
                            client.ClientData.bIsInDaimonSquareMap = true;
                            GameManager.ClientMgr.NotifyChangeMap(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client, nMapID, -1, -1, -1);
                            ...
                            GameManager.ClientMgr.SendToClient(client, strcmd, (int)TCPGameServerCmds.CMD_SPR_DAIMONSQUAREPLAYERNUMNOTIFY);

                            GameManager.ClientMgr.NotifyDaimonSquareMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, bcTmp.m_nMapCode,
                                                                    (int)TCPGameServerCmds.CMD_SPR_DAIMONSQUAREPLAYERNUMNOTIFY, 0, 0, 0, 0, bcTmp.m_nPlarerCount); // 战斗结束倒计时

                            // 每日活跃
                            DailyActiveManager.ProcessCompleteDailyActivityForDailyActive(client, (int)SpecialActivityTypes.DemoSque);

                            // 存住场景ID
                            Global.SaveRoleParamsInt32ValueToDB(client, RoleParamName.DaimonSquareSceneid, nMapID, true);
                ...
                case MagicActionIDs.GOTO_ANGELTEMPLE:   // 进入天使神殿-- 各种判断 [3/23/2014 LiaoWei]
                    {
                        if (self is GameClient)
                        {
                            ...
                            // 记条LOG
                            LogManager.WriteLog(LogTypes.Info, string.Format("{0} enter AngelTemple count={1} time={2}", client.ClientData.RoleID, nCount, DateTime.Now.ToLongDateString()));
                        }
                ...
                case MagicActionIDs.ADD_VIPEXP: // 增加VIP经验值 [4/6/2014 LiaoWei]
                    {
                        if (self is GameClient)
                        {
                            ...
                            if (nAddValue > 0)
                            {
                                ...
                                Global.SaveRoleParamsInt32ValueToDB(client, RoleParamName.VIPExp, nAddValue, false);
                ...
                case MagicActionIDs.ADD_SHENGWANG: // 增加声望值 [5/8/2014 LiaoWei]
                    {
                        if (self is GameClient)
                        {
                            ...
                            if (nAddValue > 0)
                            {
                                ...
                                string strinfo = "";
                                strinfo = string.Format("为{0}添加了声望{1}", client.ClientData.RoleName, nAddValue);
                ...
                case MagicActionIDs.ADD_XINGHUN: // 清楚指定怪物的bufferID
                    {
                        if (self is GameClient)
                        {
                            ...
                            if (nAddValue > 0)
                            {
                                ...
                                string strinfo = "";
                                strinfo = string.Format("为{0}添加了星魂{1}", client.ClientData.RoleName, nAddValue);
                                GameManager.ClientMgr.SendSystemChatMessageToClient(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool,
                                    client, strinfo);
                            }
                        }
                ...
                case MagicActionIDs.MU_FIRE_STRAIGHT: 
                    {
                        ...
                        if (gridX > 0 && gridY > 0)
                        {
                            ...
                            //if (self is Monster)
                            //{
                            //    System.Console.WriteLine(string.Format("增加BUFF效果{0}：{1}:{2}", (self as Monster).CurrentMagic, DateTime.Now.Ticks, (self as Monster).Action));
                            //}
                        }
                    }
                    break;
                ...
                case MagicActionIDs.ADDYSFM:
                    {
                        ...
                        int currPowder = Global.GetRoleParamsInt32FromDB((self as GameClient), RoleParamName.ElementPowderCount);
                        Global.SaveRoleParamsInt32ValueToDB((self as GameClient), RoleParamName.ElementPowderCount, currPowder + num, true);
                        GameManager.ClientMgr.NotifySelfParamsValueChange((self as GameClient), RoleCommonUseIntParamsIndexs.YuansuFenmo, currPowder + num);
                    }
                    break;
                default:
                    break;
