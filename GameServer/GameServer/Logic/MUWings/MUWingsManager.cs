...
namespace GameServer.Logic.MUWings
{
    ...
    public static class MUWingsManager
    {
        ...
        /// <param name="e"></param>
        public static WingData AddWingDBCommand(TCPOutPacketPool pool, int roleID, int WingID)
        {
            //先DBServer请求扣费
            TCPOutPacket tcpOutPacket = null;
            string strcmd = string.Format("{0}:{1}", roleID, WingID);
            TCPProcessCmdResults dbRequestResult = Global.RequestToDBServer2(Global._TCPManager.tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_ADDWING, strcmd, out tcpOutPacket);
        ...
        {  
            //roleid, wingDbid, using, wingid, forgeLevel, failedNum, starExp
            string strcmd = string.Format("{0}:{1}:{2}:{3}:{4}:{5}:{6}", client.ClientData.RoleID, dbID, isUsing, "*", "*", "*", "*");
            string[] fields = Global.ExecuteDBCmd((int)TCPGameServerCmds.CMD_DB_MODWING, strcmd);
        ...
        {
            //roleid, wingDbid, using, wingid, forgeLevel, failedNum, starExp
            string strcmd = string.Format("{0}:{1}:{2}:{3}:{4}:{5}:{6}", client.ClientData.RoleID, dbID,  
                                          "*",
                                          "*",
                                          nStarLevel,
                                          "*",
        ...
        /// <param name="nStarExp"></param>
        /// <returns></returns>
        public static int WingUpDBCommand(GameClient client, int dbID, int nWingLevel, int nFailNum, int nStarLevel, int nStarExp)
        {
            //roleid, wingDbid, using, wingid, forgeLevel, failedNum, starExp
            string strcmd = string.Format("{0}:{1}:{2}:{3}:{4}:{5}:{6}", client.ClientData.RoleID, dbID,
                                          "*",
