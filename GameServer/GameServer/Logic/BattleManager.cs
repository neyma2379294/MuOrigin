    ...
    public class BattleManager
    {
        ...
        private void ProcessBattling()
        {
            ...
            else if (BattlingState == BattleStates.WaitingFight) //等待战斗倒计时(此时禁止新用户进入, 此时伤害无效)
            {
                ...
                if (ticks >= (StateStartTicks + (PrepareSecs * 1000)))
                {
                    ...
                    //隋营和唐营最后一次杀敌的时间
                    _SuiLastKillEmemyTime = DateTime.Now.Ticks;
        ...
        public int TangKilledNum
        {
            ...
            { 
                ...
                {
                    //如果原有数据小，表示新增了积分，新杀了敌人
                    _TangLastKillEmemyTime = DateTime.Now.Ticks;
        ...
        private long GetBattleExpByLevel(GameClient client, int level)
        {
            ...
            if (null == expByLevels)
            {
                ...
                {
                    if (GameManager.systemBattleExpMgr.SystemXmlItemDict.TryGetValue(i, out systemXmlItem))
                    {
            ...
            if (level <= 0 || level >= BattleExpByLevels.Length)
            {
                return 0;
            }
            ...
            return (int)(expByLevels[level] * nRate);
        }
        ...
        private void ProcessRoleBattleExpAndFlagAward(GameClient client, int successSide, int paiMing)
        {
            ...
            {
                GameManager.ClientMgr.SendMailWhenPacketFull(client, goodsDataList, 
                    string.Format(Global.GetLang("恭喜您在本次阵营战中获得第{0}名，奖励已发放到附件中，请及时领取。"), paiMing + 1), Global.GetLang("阵营战排名奖励"));
            }
            else
            {
                for (int i = 0; i < goodsDataList.Count; i++)
                {
                    ...
                    Global.AddGoodsDBCommand(Global._TCPManager.TcpOutPacketPool, client, goodsDataList[i].GoodsID, goodsDataList[i].GCount, goodsDataList[i].Quality, "",
                                                goodsDataList[i].Forge_level, goodsDataList[i].Binding, 0, "", true, 1, "阵营战排名奖励", Global.ConstGoodsEndTime,
        ...
        private void ProcessTimeNotifyBattleKilledNum()
        {
            ...
            if (LastSuiKilledNum != SuiKilledNum || LastTangKilledNum != TangKilledNum)
            {
                ...
                LastTangKilledNum = SuiKilledNum;
