...
namespace GameServer.Logic
{
    ...
    public class MonsterZoneManager
    {
        ...
        /// 单次处理的最大数量
        /// </summary>
        public static int MaxRunQueueNum = 50;

        /// <summary>
        /// 最大等待的数量
        /// </summary>
        public static int MaxWaitingRunQueueNum = 100;
        ...
        /// </summary>
        private Queue<MonsterZoneQueueItem> WaitingAddDynamicMonsterQueue = new Queue<MonsterZoneQueueItem>();
        ...
        public void RunMapMonsters(SocketListener sl, TCPOutPacketPool pool)
        {
            ...
            for (int i = 0; i < FuBenMonsterZoneList.Count; i++)
            {
                ...
                MonsterZoneList[i].DestroyDeadMonsters();
            }
        ...
        public void AddDynamicRobot(int mapCode, Robot robot, int copyMapID = -1, int addNum = 1, int gridX = 0, int gridY = 0, int radius = 3, int pursuitRadius = 0, SceneUIClasses managerType = SceneUIClasses.Normal, object tag = null)
        {
            ...
            Monster _monster = monsterZone.LoadDynamicRobot(new MonsterZoneQueueItem()
                {
                    CopyMapID = copyMapID,
                    BirthCount = addNum,
        ...
        /// <param name="mapCode"></param>
        /// <param name="copyMap"></param>
        public void AddDynamicMonsters(int mapCode, int monsterID, int copyMapID = -1, int addNum = 1, int gridX = 0, int gridY = 0, int radius = 3, int pursuitRadius = 0, SceneUIClasses managerType = SceneUIClasses.Normal, object tag = null)
        {
            //先打印下怪物日志，便于调试
            TraceAllDynamicMonsters();
        ...
        /// 执行添加任意地图的动态怪物
        /// </summary>
        private bool RunAddDynamicMonsters()
        {
            MonsterZoneQueueItem monsterZoneQueueItem = null;
            ...
            lock (WaitingAddDynamicMonsterQueue) //线程锁
            {
                if (WaitingAddDynamicMonsterQueue.Count > 0)
                {
                    monsterZoneQueueItem = WaitingAddDynamicMonsterQueue.Dequeue();
                }
            }
