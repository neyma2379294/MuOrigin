...
namespace GameServer.Logic
{
    ...
    public class BloodCastleCopySceneManager
    {
        ...
        {
            /*for (int i = 0; i < Data.BloodCastleCopySceneList.Length; ++i)
            {
        ...
        {
            if (Data.BloodCastleCopySceneList == null)
                return false;
            ...
            for (int i = 0; i < Data.BloodCastleCopySceneList.Length; ++i)
            {
                nMapID = Data.BloodCastleCopySceneList[i];
        ...
        {
            int[] nMapcode = GameManager.systemParamsList.GetParamValueIntArrayByName("BloodCastleMapCodeID");
...
//         }
                ...
                if (m_BloodCastleCopyScenesInfo.TryGetValue(nFubenID, out dicTmp) && dicTmp != null)
                {
                    ...
                    if (dicTmp.TryGetValue(nSquID, out bcTmp) && bcTmp != null)
                    {
                        ...
                        else if (nCmdID == (int)TCPGameServerCmds.CMD_SPR_BLOODCASTLEPREPAREFIGHT)
                        {
                            ...
                            {
                                //System.Console.WriteLine("血色堡垒{0}里 时间没得到返回了...！！！", nSquID);
                                return;
                            }
                            ...
                            //System.Console.WriteLine("血色堡垒{0}里 时间{1}！！！", nSquID, nTimer);
        ...
        public int EnterBloodCastSceneCopyScene(GameClient client, int nFubenID, int nBloodNum, out int nSeqID, int mapCode)
        {
            ...
            lock (m_BloodCastleCopyScenesInfo)
            {
                if (m_BloodCastleCopyScenesInfo.TryGetValue(nFubenMapID, out dicTmp) && dicTmp != null)
                {
                    ...
                    {
                        if (bcsceneInfo.Value.m_eStatus >= BloodCastleStatus.FIGHT_STATUS_END)
                        {
                            continue;
                        }
            ...
            Global.UpdateDayActivityEnterCountToDB(client, client.ClientData.RoleID, nDate, nType, ++nBloodNum);
                    ...
                    lock (m_BloodCastleCopyScenesInfo)
                    {
                        ...
                        if (bcTmp.m_eStatus == BloodCastleStatus.FIGHT_STATUS_NULL)             // 如果处于空状态 -- 是否要切换到准备状态
                        {
                            ...
                            if (Global.CanEnterBloodCastleCopySceneOnTime(bcDataTmp.BeginTime, bcDataTmp.PrepareTime + bcDataTmp.DurationTime, out nSecond, out strTimer))
                            {
                                ...
                                //System.Console.WriteLine("剩余时间为{0}...！！！", nSecond);

                                DateTime staticTime = DateTime.Parse(strTimer);

                                bcTmp.m_lPrepareTime = staticTime.Ticks / 10000;//DateTime.Now.Ticks / 10000;
                        ...
                        else if (bcTmp.m_eStatus == BloodCastleStatus.FIGHT_STATUS_END)         // 战斗结束
                        {
                            ...
                            if (ticks >= (bcTmp.m_lEndTime + (bcDataTmp.LeaveTime * 1000) + 3000))
                            {
                                try
                                {
                                    // 清场
                                    ...
                                    {
                                        string log = string.Format("血色城堡已结束,是否完成{0},结束时间{1},m_bIsFinishTask:{2},m_nKillMonsterACount:{3},m_bKillMonsterAStatus:{4},m_nRoleID:{5},踢出玩家列表:",
                                            bcTmp.m_bIsFinishTask, new DateTime(bcTmp.m_lEndTime * 10000), bcTmp.m_bIsFinishTask,
                                            bcTmp.m_nKillMonsterACount, bcTmp.m_bKillMonsterAStatus, bcTmp.m_nRoleID);
                                        ...
                                        for (int n = 0; n < objsList.Count; ++n)
                                        {
                                            ...
                                            if (c.ClientData.FuBenID < 0 || !IsBloodCastleCopyScene(c.ClientData.FuBenID))
                                                continue;
                                ...
                                {
                                    DataHelper.WriteExceptionLogEx(ex, "血色城堡调度异常");
                                }
                            }
                        }
                    }
        ...
        /// 玩家离开血色堡垒
        /// </summary>
        public void LeaveBloodCastCopyScene(GameClient client)
        {
            int nFuBenId = -1;
