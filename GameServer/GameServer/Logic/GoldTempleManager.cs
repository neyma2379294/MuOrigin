...
namespace GameServer.Logic
{
    ...
    public class GoldTempleManager
    {
        ...
        {
            long lTicks = DateTime.Now.Ticks / 10000;
            if (lTicks >= (m_SubMoneyTick + m_SubMoneyInterval))
            {
                m_SubMoneyTick = lTicks;

                if (!GameManager.ClientMgr.SubUserMoney(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool,
                                                                client, Data.GoldtempleData.OneMinuteNeedDiamond, "黄金神庙扣除"))
                {
                    KickOutScene(client);
                }
