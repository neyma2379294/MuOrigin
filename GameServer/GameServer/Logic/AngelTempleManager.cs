    ...
    public class AngelTempleManager
    {
        ...
        public void ProcessAttackBossInAngelTempleScene(GameClient client, Monster monster, int nDamage)
        {
            ...
            lock (m_PointDamageInfoMutex)
            {
                ...
                {
                    m_RoleDamageAngelValue[client.ClientData.RoleID].m_DamagePoint += nDamage;

                    tmpInfo = m_RoleDamageAngelValue[client.ClientData.RoleID];                
                }
        ...
        public void GiveAwardAngelTempleScene(bool bBossKilled)
        {
            ...
            foreach (var dp in pointList)
            {
                ...
                if (null != gc)
                {
                    ...
                    foreach (var item in dp.GoodsList.Items)
                    {
                        Global.AddGoodsDBCommand(Global._TCPManager.TcpOutPacketPool, gc, item.GoodsID, item.GoodsNum, 0, "", item.Level, item.Binding , 0,
                                                        "", true, 1, "天使神殿奖励物品", Global.ConstGoodsEndTime, 0, 0, item.IsHaveLuckyProp, 0, item.ExcellencePorpValue, item.AppendLev);
                    }
        ...
        public bool AddBuffer(GameClient client, BufferItemTypes buffID, double[] newParams, bool notifyPropsChanged)
        {
            ...
            {
                bufferData1 = Global.GetBufferDataByID(client, (int)BufferItemTypes.MU_ANGELTEMPLEBUFF2);
            ...
            {
                bufferData1 = Global.GetBufferDataByID(client, (int)BufferItemTypes.MU_ANGELTEMPLEBUFF1);
