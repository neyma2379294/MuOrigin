...
namespace GameServer.Logic
{
    ...
    public class BroadcastInfoMgr
    {
        ...
        /// 上次播放的天
        /// </summary>
        private static int LastBroadcastDay = DateTime.Now.DayOfYear;

        /// <summary>
        /// 上一次广播的时间记录
        /// </summary>
        private static BroadcastTimeItem LastBroadcastTimeItem = new BroadcastTimeItem() { Hour = DateTime.Now.Hour, Minute = DateTime.Now.Minute, };
        ...
        public static void LoginBroadcastInfos(GameClient client)
        {
            ...
            for (int i = 0; i < broadcastInfoItemList.Count; i++)
            {
                ...
                if (broadcastInfoItemList[i].KaiFuStartDay > 0) //根据开服后几天来播报
                {
                    ...
                    if ((int)todayTime.DayOfYear - (int)jugeDateTime.DayOfYear >= broadcastInfoItemList[i].KaiFuStartDay)
                    {
                        if (broadcastInfoItemList[i].KaiFuShowType > 0)
                        {
                            if (todayTime.DayOfYear - jugeDateTime.DayOfYear < (broadcastInfoItemList[i].KaiFuStartDay + broadcastInfoItemList[i].KaiFuShowType))
                            {
                                ///可以播报
                            }
                            else
