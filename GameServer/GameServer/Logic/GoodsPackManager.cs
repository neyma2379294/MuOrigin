...
namespace GameServer.Logic
{
    ...
    public class GoodsPackManager
    {
        ...
        {
            Interlocked.Add(ref BaseGoodsID, 1);

            int n = 0;
            Interlocked.Exchange(ref n, BaseGoodsID);
            return n;
        }
        ...
        /// 角色的掉落包ID
        /// </summary>
        private int BaseRoleGoodsPackID = 0;
        ...
        {
            Interlocked.Add(ref BaseRoleGoodsPackID, 1);

            int n = 0;
            Interlocked.Exchange(ref n, BaseRoleGoodsPackID);
            return n;
        }
        ...
        public List<FallGoodsItem> GetFallGoodsItemByPercent(List<FallGoodsItem> gallGoodsItemList, int maxFallCount, int fallAlgorithm)
        {
            ...
            if ((int)FallAlgorithm.MonsterFall == fallAlgorithm)
            {
                for (int i = 0; i < gallGoodsItemList.Count; i++)
                {
                    ...
                    if (randPercent <= gallGoodsItemList[i].SelfPercent)
                    {
        ...
        /// <param name="goodsPackID"></param>
        /// <returns></returns>
        public List<GoodsData> GetGoodsDataList(IObject attacker, int goodsPackID, int maxFallCount, int forceBinding)
        {
            ...
            List<FallGoodsItem> tempItemList2 = GetFallGoodsItemByPercent(fallGoodsItemList, maxFallCount, (int)FallAlgorithm.MonsterFall);
        ...
        /// <param name="goodsPackID"></param>
        /// <returns></returns>
        private List<GoodsData> GetLimitTimeGoodsDataList(IObject attacker, int goodsPackID, int maxFallCount, int forceBinding)
        {
            List<GoodsData> goodsDataList = null;
        ...
        private GoodsPackItem GetMonsterGoodsPackItem(GameClient client, int ownerRoleID, string ownerRoleName, int goodsPackID, List<int> teamRoleIDs, int mapCode, int copyMapID, int toX, int toY, int forceBinding, string monsterName, int belongTo, int fallLevel, int teamID)
        {
            ...
            GoodsPackItem goodsPackItem = new GoodsPackItem()
            {
                ...
                GoodsPackType = 0,
                ProduceTicks = DateTime.Now.Ticks / 10000,
                LockedRoleID = -1,
        ...
        /// <param name="goodsPackID"></param>
        /// <returns></returns>
        public List<GoodsPackItem> GetMonsterGoodsPackItemList(IObject attacker, int ownerRoleID, string ownerRoleName, int goodsPackID, List<int> teamRoleIDs, int mapCode, int copyMapID, int toX, int toY, int forceBinding, string monsterName, int belongTo, int fallLevel, int teamID)
        {
            //根据物品掉落包ID来获取掉落的单个个数配置
            int maxFallCountByID = GetFallGoodsMaxCount(goodsPackID);
            if (maxFallCountByID <= 0)
            ...
            // 根据物品掉落ID获取要掉落的物品
            List<GoodsData> goodsDataList = GetGoodsDataList(attacker, goodsPackID, maxFallCountByID, forceBinding);
            ...
            List<GoodsData> goodsDataListLimitTime = GetLimitTimeGoodsDataList(attacker, goodsPackID, maxFallCountByID, forceBinding);
        ...
        private static bool ProcessFallTongQian(TCPOutPacketPool pool, GameClient client, int goodsID, int goodsNum, int fallLevel)
        {
            ...
            for (int i = 0; i < goodsNum; i++)
            {
                ...
                GameManager.ClientMgr.AddUserYinLiang(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool,
                    pool, client, money, "拾取金币");
            }
        ...
        public bool AutoAddThingIntoBag(SocketListener sl, TCPOutPacketPool pool, GameClient client, GoodsPackItem goodsPackItem, GoodsData goodsData)
        {
            ...
            if (Global.CanAddGoods(gc, goodsData.GoodsID, 1, goodsData.Binding) || IsFallTongQianGoods(goodsData.GoodsID))
            {
                ...
                if (!ProcessFallTongQian(pool, gc, goodsData.GoodsID, goodsData.GCount, goodsPackItem.FallLevel))
                {
                    ...
                    if (0 == nRet)
                    {
                        GameManager.logDBCmdMgr.AddDBLogInfo(/*Convert.ToInt32(goodsData.Id)*/-1, Global.ModifyGoodsLogName(goodsData), "杀怪掉落后自动拾取", Global.GetMapName(client.ClientData.MapCode), "系统", "销毁", goodsData.GCount, client.ClientData.ZoneID, client.strUserID);
                    }
                }
        ...
        public void ProcessMonsterByClient(SocketListener sl, TCPOutPacketPool pool, GameClient client, Monster monster)
        {
            ...
            List<GoodsPackItem> goodsPackItemList = GetMonsterGoodsPackItemList(client, client.ClientData.RoleID, Global.FormatRoleName(client, client.ClientData.RoleName), monster.MonsterInfo.FallGoodsPackID, teamRoleIDs,
                monster.CurrentMapCode, monster.CurrentCopyMapID, (int)grid.X, (int)grid.Y, forceBinding, monster.MonsterInfo.VSName, monster.MonsterInfo.FallBelongTo, monster.MonsterInfo.VLevel, teamID);
            ...
            lock (_GoodsPackDict)
            {
                if (_GoodsPackDict.TryGetValue(autoID, out goodsPackItem))
                {
                    if (openState > 0) //打开操作
                    {
                        ...
                        if (CanOpenGoodsPack(goodsPackItem, client.ClientData.RoleID))
                        {
                            if (-1 == goodsPackItem.LockedRoleID || goodsPackItem.LockedRoleID == client.ClientData.RoleID)
                            {
                                ...
                                goodsPackItem.OpenPackTicks = DateTime.Now.Ticks / 10000;
                    ...
                    else
                    {
                        long lastOpenPackTicks = 0;
                        goodsPackItem.RolesTicksDict.TryGetValue(client.ClientData.RoleID, out lastOpenPackTicks);
                        goodsPackItem.RolesTicksDict[client.ClientData.RoleID] = lastOpenPackTicks + (DateTime.Now.Ticks / 10000 - goodsPackItem.OpenPackTicks);
            ...
            for (int i = 0; i < goodsDataList.Count; i++)
            {
                ...
                if (Global.CanAddGoods(gc, goodsDataList[i].GoodsID, 1, goodsDataList[i].Binding) || IsFallTongQianGoods(goodsDataList[i].GoodsID))
                {
                    ...
                    if (!ProcessFallTongQian(pool, gc, goodsDataList[i].GoodsID, goodsDataList[i].GCount, goodsPackItem.FallLevel) && Global.CanAddGoodsNum(gc, goodsDataList[i].GCount))
                    {
                        ...
                        if (0 == nRet)
                        {
                            GameManager.logDBCmdMgr.AddDBLogInfo(/*Convert.ToInt32(goodsDataList[i].Id)*/-1, Global.ModifyGoodsLogName(goodsDataList[i]), "杀怪掉落后手动拾取", Global.GetMapName(client.ClientData.MapCode), "系统", "销毁", goodsDataList[i].GCount, client.ClientData.ZoneID, client.strUserID);
                        }
                    }
        ...
        private void TakeFallGoodsRecords(GoodsPackItem goodsPackItem, GameClient client)
        {
            ...
            GameManager.logDBCmdMgr.AddDBLogInfo(goodsData.Id, Global.ModifyGoodsLogName(goodsData), "拾取物品", Global.GetMapName(client.ClientData.MapCode), client.ClientData.RoleName, "增加", goodsData.GCount, client.ClientData.ZoneID, client.strUserID);
