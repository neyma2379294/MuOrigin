...
namespace GameServer.Logic
{
    ...
    /// 王城战管理
    /// </summary>
    public class LuoLanChengZhanManager
    {
        #region 内存中的王族帮会ID
        ...
        /// 王城战期间帮会独占皇宫的保持时间
        /// </summary>
        private static long BangHuiTakeHuangGongTicks = DateTime.Now.Ticks / 10000;
        ...
        public static void ProcessWangChengZhanResult()
        {
            ...
            if (WangChengZhanStates.None == WangChengZhanState) //非战斗状态
            {
                if (IsInWangChengFightingTime())
                {
                    ...
                    BangHuiTakeHuangGongTicks = DateTime.Now.Ticks / 10000;
        ...
        {
            //皇宫地图中活着的玩家列表
            List<GameClient> lsClients = GameManager.ClientMgr.GetMapAliveClients(Global.GetHuangGongMapCode());
        ...
        public static Dictionary<int, int> GetWarRequstMap(String warReqString)
        {
            ...
            for (int n = 0; n < reqItems.Length; n++)
            {
                ...
                if (day < DateTime.Now.DayOfYear && day + 120 > DateTime.Now.DayOfYear) continue;
        ...
        public static int SetCityWarRequestToDBServer(int lingDiID, String nowWarRequest)
        {
            ...
            String[] fields = Global.ExecuteDBCmd((int)TCPGameServerCmds.CMD_DB_SETLINGDIWARREQUEST, strcmd);
        ...
        public static long GetBattleEndMs()
        {
            ...
            int endMinute = 0;
            Global.JugeDateTimeInTimeRange(DateTime.Now, WangChengZhanFightingDayTimes, out endMinute, true);
