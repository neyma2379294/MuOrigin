...
namespace GameServer.Logic
{
    ...
    public class GuildCopyMapManager
    {
        ...
        public bool IsPrepareResetTime()
        {
            ...
            DateTime beginTime = DateTime.Parse("0:00");
            DateTime EndTime = DateTime.Parse("0:01");
        ...
        public void CheckCurrGuildCopyMap(GameClient client, out int fubenid, out int seqid, int mapcode)
        {
            ...
            GuildCopyMapDB data = GameManager.GuildCopyMapDBMgr.FindGuildCopyMapDB(guildid);
            // 没有帮会 否则不应该为空  
            if (null == data)
                return;
            ...
            if (Global.BeginOfWeek(openTime) != Global.BeginOfWeek(DateTime.Now))
            {
                // 重置数据库信息 并使用第一个副本id 让玩家重新开始
                GameManager.GuildCopyMapDBMgr.ResetGuildCopyMapDB(guildid);
            ...
            if (data.State == (int)GuildCopyMapState.Passed)
            {
                ...
                data.OpenDay = Global.GetOffsetDay(DateTime.Now);

                if (GameManager.GuildCopyMapDBMgr.UpdateGuildCopyMapDB(data))
    ...
    public class GuildCopyMapDBManager
    {
        ...
        /// 根据帮会id查找帮会副本信息
        /// </summary>
        public GuildCopyMapDB FindGuildCopyMapDB(int guildid)
        {
            ...
            { 
                ...
                { 
                    ...
                    TCPProcessCmdResults dbRequestResult = Global.RequestToDBServer(Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool
                        , (int)TCPGameServerCmds.CMD_SPR_GETBANGHUIFUBEN, strDbCmd, out dbFields);
        ...
        {
            // 查找现有信息 如果存在直接返回
            GuildCopyMapDB oldData = FindGuildCopyMapDB(data.GuildID);
            if (null == oldData)
            {
            ...
            { 
                ...
                TCPProcessCmdResults dbRequestResult = Global.RequestToDBServer(Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool
                    , (int)TCPGameServerCmds.CMD_DB_UPDATEBANGHUIFUBEN, strDbCmd, out dbFields);
                if (dbRequestResult == TCPProcessCmdResults.RESULT_FAILED)
                {
        ...
        /// 重置副本信息
        /// </summary>
        public void ResetGuildCopyMapDB(int guildid)
        {
            GuildCopyMapDB data = new GuildCopyMapDB() 
            {
                ...
                OpenDay = Global.GetOffsetDay(DateTime.Now),
                Killers = "",
            };
            UpdateGuildCopyMapDB(data);
        }
    }
}
