...
namespace GameServer.Logic
{
    ...
    public  class RechargeRepayActiveMgr
    {
        ...
        public static TCPProcessCmdResults QueryAllRechargeRepayActiveInfo(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            TCPProcessCmdResults retcmd = Global.RequestToDBServer(tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_QUERY_REPAYACTIVEINFO, string.Format("{0}:{1}", roleID, (int)ActivityTypes.TotalConsume), out dbFields);
            if (null == dbFields)
                return TCPProcessCmdResults.RESULT_FAILED;
        ...
        public static TCPProcessCmdResults QueryRechargeRepayActive(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool , TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            switch((ActivityTypes)activeid)
            {
                ...
                case ActivityTypes.MeiRiChongZhiHaoLi:
                    {
                        ...
                        TCPProcessCmdResults retcmd = Global.RequestToDBServer(tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_QUERY_REPAYACTIVEINFO, string.Format("{0}:{1}", fields[0], (int)ActivityTypes.MeiRiChongZhiHaoLi), out dbFields);
                        if (null == dbFields)
                            return TCPProcessCmdResults.RESULT_FAILED;
                        if (dbFields.Length != 3)
                            return TCPProcessCmdResults.RESULT_FAILED;
                ...
                case ActivityTypes.TotalCharge:
                    {
                        totalChongZhiMoney = GameManager.ClientMgr.QueryTotaoChongZhiMoney(client);
                        totalChongZhiMoney = Global.TransMoneyToYuanBao(totalChongZhiMoney);
                        TCPProcessCmdResults retcmd = Global.RequestToDBServer(tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_QUERY_REPAYACTIVEINFO, string.Format("{0}:{1}", fields[0], (int)ActivityTypes.TotalCharge), out dbFields);
                        if (null == dbFields)
                            return TCPProcessCmdResults.RESULT_FAILED;
                        if (dbFields.Length != 3)
                            return TCPProcessCmdResults.RESULT_FAILED;
                ...
                case ActivityTypes.TotalConsume:
                    {
                        TCPProcessCmdResults retcmd = Global.RequestToDBServer(tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_QUERY_REPAYACTIVEINFO, string.Format("{0}:{1}", fields[0], fields[1]), out dbFields);
                        if (null == dbFields)
                            return TCPProcessCmdResults.RESULT_FAILED;
                        if (dbFields.Length != 3)
                            return TCPProcessCmdResults.RESULT_FAILED;
                ...
                case ActivityTypes.HeFuRecharge:
                    {
                        ...
                        { 
                            ...
                            { 
                                // 向DB申请数据
                                TCPProcessCmdResults retcmd = Global.RequestToDBServer(tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_QUERY_REPAYACTIVEINFO, string.Format("{0}:{1}:{2}:{3}", fields[0], fields[1], hefuday, instance.strcoe), out dbFields);
                                if (null == dbFields)
                                    return TCPProcessCmdResults.RESULT_FAILED;
        ...
        public static bool CheckRechargeReplay(GameClient client, ActivityTypes type, out bool hasGet)
        {
            ...
            try
            {
                ...
                //        {
                            string[] dbFields = null;
                            TCPProcessCmdResults retcmd = Global.RequestToDBServer(Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool, (int)TCPGameServerCmds.CMD_DB_QUERY_REPAYACTIVEINFO, string.Format("{0}:{1}", client.ClientData.RoleID, (int)type), out dbFields);
                    ...
                    case ActivityTypes.TotalCharge:
                        {
                            ...
                            string writerec = RechargeRepayActiveMgr.BuildWriteActiveRecordStr(dbFields[1], nBtnIndex);
                            Global.RequestToDBServer(tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_GET_REPAYACTIVEAWARD, string.Format("{0}:{1}:{2}", nRoleID, (int)ActivityTypes.TotalCharge, writerec.Replace(",","")), out dbFields2);
                            if (dbFields2 == null || dbFields2.Length != 3)
                                return TCPProcessCmdResults.RESULT_FAILED;
                    ...
                    case ActivityTypes.TotalConsume:
                        {
                            ...
                            string[] dbFields=null;
                            TCPProcessCmdResults retcmd = Global.RequestToDBServer(tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_QUERY_REPAYACTIVEINFO, string.Format("{0}:{1}", nRoleID, (int)ActivityTypes.TotalConsume), out dbFields);
                            if(dbFields==null)
                                return TCPProcessCmdResults.RESULT_FAILED;
                            if (dbFields != null && dbFields.Length != 3)
                                return TCPProcessCmdResults.RESULT_FAILED;
                    ...
                    case ActivityTypes.HeFuRecharge:
                        {
                            ...
                            string[] dbFields = null;
                            Global.RequestToDBServer(tcpClientPool, pool, (int)TCPGameServerCmds.CMD_DB_GET_REPAYACTIVEAWARD, string.Format("{0}:{1}:{2}:{3}", nRoleID, (int)tmpActType, hefuday, instance.strcoe), out dbFields);
                            ...
                            if (!GameManager.ClientMgr.AddUserMoney(Global._TCPManager.MySocketListener, Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool,
                                 client, userrebate, string.Format(Global.GetLang("领取{0}活动奖励"), instance.ActivityType), ActivityTypes.HeFuRecharge))
                            {
                                // 没有可领取的充值返利
                                strcmd = string.Format("{0}:{1}::", (int)ActivityErrorType.NOTCONDITION, nActivityType);
                                tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strcmd, nID);
                            ...
                            GameManager.ClientMgr.NotifyImportantMsg(Global._TCPManager.MySocketListener, Global._TCPManager.TcpOutPacketPool, client,
                                                                                           StringUtil.substitute(Global.GetLang("恭喜获得钻石 +" + userrebate)),
