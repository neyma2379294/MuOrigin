...
namespace GameServer.Logic
{
    ...
    public class CopyMapManager
    {
        ...
        /// <param name="mapCode"></param>
        /// <returns></returns>
        private int FindCopyID(int fuBenSeqID, int mapCode)
        {
            int copyMapID = -1;
            string key = string.Format("{0}_{1}", fuBenSeqID, mapCode);
            lock (FuBenSeqID2CopyIDDict)
        ...
        private bool CanRemoveCopyMap(CopyMap copyMap, long nowTicks)
        {
            ...
            if ((nowTicks - lastLeaveClientTicks) < (60 * 3 * 1000))
            {
                //还未到删除的时间
                return false;
            }
        ...
        {
            if (CopyTeamManager.getInstance().NeedRecordDamageInfoFuBenID(copyMap.FubenMapID))
            {
                    ...
                    if (RoleDamageDict.TryGetValue(copyMap.CopyMapID, out dict))
                    {
                        ...
                        foreach (var client in clientList)
                        {
                            ...
                            if (damage > 0)
                            {
                                ...
                                {
                                    dict.Add(roleID, new RoleDamage(roleID, damage));
                                }
                                sumdamage += damage;
                            }
                        }
                ...
                if (null != fuBenInfoItem)
                {
                    ...
                    if (fuBenID > 0)
                    {
                        ...
                        if (fuBenMapItem.Experience > 0 && fuBenMapItem.Money1 > 0)
                        {
                            ...
                            //向客户的发放通关奖励
                            fubenTongGuanData = Global.GiveCopyMapGiftForScore(client, fuBenID, nMaxTime, nFinishTimer, killedNum, nDieCount, fuBenMapItem.Experience, fuBenMapItem.Money1, fuBenMapItem, strName);
                    ...
                    if (fuBenID > 0)
                    {
                        ...
                        if (fuBenMapItem.Experience > 0 && fuBenMapItem.Money1 > 0)
                        {
                            ...
                            for (int i = 0; i < objsList.Count; i++)
                            {
                                ...
                                //向客户的发放通关奖励
                                fubenTongGuanData = Global.GiveCopyMapGiftForScore(c, fuBenID, nMaxTime, nFinishTimer, killedNum, nDieCount, fuBenMapItem.Experience, fuBenMapItem.Money1, fuBenMapItem, strName);
                            }
                        }
            ...
            if (fuBenSeqID > 0) //如果副本不存在
            {
                ...
                if (null != fuBenInfoItem)
                {
                    ...
                    if (fuBenID > 0)
                    {
                        ...
                        fubenTongGuanData.FuBenID = usedSecs;
