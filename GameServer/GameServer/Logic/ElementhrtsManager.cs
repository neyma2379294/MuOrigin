...
namespace GameServer.Logic
{
    ...
    class ElementhrtsManager
    {
        ...
        /// 元素背包的最大格子数
        /// </summary>
        public static int MaxElementhrtsGridNum = 10;

        /// <summary>
        /// 元素装备栏最大格子数
        /// </summary>
        public static int MaxUsingElementhrtsGridNum = 10;
        ...
        public static void LoadRefineType()
        {
            ...
            if (null == xml)
            {
                System.Console.WriteLine("        加载Config/RefineType.xml时出错!!!文件不存在");
                return;
            }
            ...
            {
                System.Console.WriteLine("        加载Config/ElementsHeart.xml时出现异常!!!");
            }
        }
        ...
        public static void LoadElementHrtsBase()
        {
            ...
            if (null == xml)
            {
                System.Console.WriteLine("        加载Config/Refine.xml时出错!!!文件不存在");
                return;
            }
        ...
        public static void LoadElementHrtsLevelInfo()
        {
            ...
            if (null == xml)
            {
                System.Console.WriteLine("        加载Config/ElementsHeart.xml时出错!!!文件不存在");
                return;
            }
                ...
                lock (ElementHrtsLevelDict)
                {
                    ...
                    foreach (var xmlItem in xmlItems)
                    {
                        ...
                        foreach (var arg in args)
                        {
                            ...
                            {
                                System.Console.WriteLine(string.Format("        加载Config/ElementsHeart.xml时出错!!!，{0}, {1}", ID, Level));
                                return;
                            }
        ...
        public static void LoadSpecialElementHrtsExp()
        {
            ...
            lock (SpecialExpDict)
            {
                ...
                if (null == strParam)
                {
                    System.Console.WriteLine("        SpecialElementsHeart 不存在，加载失败");
                    return ;
                }
        ...
        private static void RequestElementHrtList(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, GameClient client, out TCPOutPacket tcpOutPacket)
        {
            ...
            TCPProcessCmdResults result = Global.TransferRequestToDBServer(tcpMgr, socket, tcpClientPool, tcpRandKey, pool, (int)TCPGameServerCmds.CMD_GETGOODSLISTBYSITE, bytesCmd, bytesCmd.Length, out tcpOutPacket);
        ...
        public static TCPProcessCmdResults ProcessUseElementHrt(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                int roleID = 1000000;//Convert.ToInt32(fields[0]);
                ...
                if (bEquip)
                {
                    ...
                    if (null == goodsData)
                    {
                        strCmd = string.Format("{0}:{1}", (int)ElementhrtsError.ItemNotExist, roleID);
                        tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                        return TCPProcessCmdResults.RESULT_DATA;
                    }
                    ...
                    if ((int)SaleGoodsConsts.UsingElementhrtsGoodsID == goodsData.Site)
                    {
                        strCmd = string.Format("{0}:{1}", (int)ElementhrtsError.AlreadyUse, roleID);
                        tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                        return TCPProcessCmdResults.RESULT_DATA;
                    }
                    ...
                    {
                        strCmd = string.Format("{0}:{1}", (int)ElementhrtsError.EquipIsFull, roleID);
                        tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                        return TCPProcessCmdResults.RESULT_DATA;
                    }
                ...
                else
                {
                    ...
                    if (null == goodsData)
                    {
                        strCmd = string.Format("{0}:{1}:{2}:{3}", (int)ElementhrtsError.ItemNotExist, roleID, 0, 0);
                        tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                        return TCPProcessCmdResults.RESULT_DATA;
                    }
                    ...
                    if ((int)SaleGoodsConsts.UsingElementhrtsGoodsID != goodsData.Site)
                    {
                        strCmd = string.Format("{0}:{1}:{2}:{3}", (int)ElementhrtsError.AlreadyOut, roleID, 0, 0);
                        tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                        return TCPProcessCmdResults.RESULT_DATA;
                    }
                    // 检查元素背包是否有空位
                    ...
                    {
                        strCmd = string.Format("{0}:{1}:{2}:{3}", (int)ElementhrtsError.BagIsFull, roleID, 0, 0);
                        tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                        return TCPProcessCmdResults.RESULT_DATA;
                    }
                ...
                string strcmd = string.Format("{0}:{1}:{2}:{3}", (int)ElementhrtsError.Success, roleID, 0, 0);
                tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strcmd, nID);
        ...
        public static TCPProcessCmdResults ProcessGetSomeElementHrts(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                {
                    int GoodsID = 0;
                    ElementhrtsError result = GetSomeElementHrts(client, bUseMoney, out GoodsID);
                    if (ElementhrtsError.Success != result)
                    {
        ...
        public static ElementhrtsError GetSomeElementHrts(GameClient client, bool bUseMoney, out int GoodsID)
        {
            GoodsID = 0;
            // 当前档次
            int currGrade = Global.GetRoleParamsInt32FromDB(client, RoleParamName.ElementGrade);
            ...
            {
                currPowder = 999999;
                Global.SaveRoleParamsInt32ValueToDB(client, RoleParamName.ElementPowderCount, currPowder, true);
            }
            ...
            if (refinecfg.RefineCost > 0)
            {
                Global.SaveRoleParamsInt32ValueToDB(client, RoleParamName.ElementPowderCount, currPowder - refinecfg.RefineCost, true);
            }
        ...
        public static TCPProcessCmdResults ProcessPowerElementHrt(TCPManager tcpMgr, TMSKSocket socket, TCPClientPool tcpClientPool, TCPRandKey tcpRandKey, TCPOutPacketPool pool, int nID, byte[] data, int count, out TCPOutPacket tcpOutPacket)
        {
            ...
            try
            {
                ...
                string[] strMaterials = fields[2].Split('|');
                ...
                GoodsData goodsData = GetElementhrtsByDbID(client, srcid);
                if (null == goodsData)
                {
                    strCmd = string.Format("{0}:{1}:{2}:{3}:{4}", (int)ElementhrtsError.ErrorParams, roleID, 0, 0, 0);
                    tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                ...
                {
                    strCmd = string.Format("{0}:{1}:{2}:{3}:{4}", (int)ElementhrtsError.GoodsNotExist, roleID, 0, 0, 0);
                    tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                    return TCPProcessCmdResults.RESULT_DATA;
                }
                ...
                if (categoriy == (int)ItemCategories.SpecialElementHrt)
                {
                    strCmd = string.Format("{0}:{1}:{2}:{3}:{4}", (int)ElementhrtsError.CantEquip, roleID, 0, 0, 0);
                    tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strCmd, nID);
                    return TCPProcessCmdResults.RESULT_DATA;
                }
                ...
                foreach (var item in materialList)
                {
                    ...
                    else
                    {
                        ...
                        {
                            ElementHrtsLevelInfo materialInfo = GetElementHrtsLevelInfo(systemMaterial.GetIntValue("SuitID"), goodsData.ElementhrtsProps[0]/*level*/);
                            if (null != materialInfo)
                            {
                                exp = materialInfo.TotalExp + goodsData.ElementhrtsProps[1];
                            }
                        }
                    }
                ...
                while (addExp > 0)
                {
                    ElementHrtsLevelInfo info = GetElementHrtsLevelInfo(SuitID, currLevel);
                    if (null == info)
                    {
                        break;
                    }
                ...
                string strcmd = string.Format("{0}:{1}:{2}:{3}:{4}", (int)ElementhrtsError.Success, roleID, srcid, currLevel, currExp);
                tcpOutPacket = TCPOutPacket.MakeTCPOutPacket(pool, strcmd, nID);
