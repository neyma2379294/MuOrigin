...
namespace GameServer.Logic.RefreshIconState
{
    ...
    public class TimerBossManager
    {
                ...
                foreach (KeyValuePair<int, int> kvp in m_LivedInMapBoss)
                {
                    if (kvp.Value == (int)MonsterBirthTypes.TimePoint)
                    {
                        ...
                        if (m_WorldBossDict.TryGetValue(kvp.Key, out bossData))
                        {
                            ...
                            {
                                // 达到要求的转生数与级别
                                if ((client.ClientData.ChangeLifeCount >= bossData.nReqChangeLiveCount
                                    && client.ClientData.Level >= bossData.nReqLevel)
        ...
        {
            List<GameClient> allClient = GameManager.ClientMgr.GetAllClients();
            ...
            if (null != allClient)
            {
                for (int i = 0; i < allClient.Count(); i++)
                {
                    if (null != allClient[i])
                    {
                        allClient[i]._IconStateMgr.CheckHuangJinBoss(allClient[i]);
                        allClient[i]._IconStateMgr.SendIconStateToClient(allClient[i]);
                    }
                }
            }
        }
        ...
        public void RefreshWorldBoss()
        {
            ...
            if (null != allClient)
            {
                for (int i = 0; i < allClient.Count(); i++)
                {
                    if (null != allClient[i])
                    {
                        allClient[i]._IconStateMgr.CheckShiJieBoss(allClient[i]);
                        allClient[i]._IconStateMgr.SendIconStateToClient(allClient[i]);
                    }
                }
            }
