    ...
    public class JunQiManager
    {
        ...
        public static void AddKillJunQiItem(int mapCode, int npcID, int bhid)
        {
            ...
            lock (KillJunQiDict)
            {
                ...
                {
                    BHID = bhid,
                    KillJunQiTicks = DateTime.Now.Ticks / 10000,
                };
            }
        }
        ...
        /// </summary>
        /// <param name="bhid"></param>
        public static bool CanInstallJunQiNow(int mapCode, int npcID, int bhid)
        {
        ...
        public static void LoadBangHuiJunQiItemsDictFromDBServer()
        {
            ...
            if (TCPProcessCmdResults.RESULT_FAILED == Global.RequestToDBServer3(Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool,
                (int)TCPGameServerCmds.CMD_DB_GETBHJUNQILIST, string.Format("{0}", GameManager.ServerLineID), out bytesData))
            {
        ...
        public static void LoadBangHuiLingDiItemsDictFromDBServer()
        {
            byte[] bytesData = null;
            if (TCPProcessCmdResults.RESULT_FAILED == Global.RequestToDBServer3(Global._TCPManager.tcpClientPool, Global._TCPManager.TcpOutPacketPool,
                (int)TCPGameServerCmds.CMD_DB_GETBHLINGDIDICT, string.Format("{0}", GameManager.ServerLineID), out bytesData))
            {
                return; //如果查询失败，就当做时不在线了
            }
        ...
        /// <returns></returns>
        private static JunQiItem AddJunQi(int mapCode, int bhid, int zoneID, string bhName, int npcID, string junQiName, int junQiLevel)
        {
        ...
        /// 处理添加帮旗
        /// </summary>
        public static void ProcessNewJunQi(SocketListener sl, TCPOutPacketPool pool, int mapCode, int bhid, int zoneID, string bhName, int npcID, string junQiName, int junQiLevel)
        {
            //添加一个新的帮旗数据
            JunQiItem JunQiItem = AddJunQi(mapCode, bhid, zoneID, bhName, npcID, junQiName, junQiLevel);
            if (null == JunQiItem)
            {
        ...
        private static void ProcessJunQiDead(SocketListener sl, TCPOutPacketPool pool, GameClient client, JunQiItem junQiItem)
        {
            ...
            junQiItem.JunQiDeadTicks = DateTime.Now.Ticks / 10000;
        ...
        /// 最大的插旗数量
        /// </summary>
        private static int MaxInstallQiNum = 1;
